...
- 2026-07-13 — Project renamed "Bliwit" → "Bleevit" (user-requested): literal replace across the same 10 living/derived files. `docs/architecture/` still contains zero occurrences, so no frozen-doc amendment was needed. Prior log lines above referencing "Bliwit" are left as historical record.
...
| 2026-07-13 | — (rename, unplanned) | Renamed project "Bliwit" → "Bleevit" across all living/derived docs (same 10 files as the prior rename). Confirmed `docs/architecture/` has no occurrences, so R-1 change control did not apply. | M0 — repo bootstrap incl. V-1 pin re-verification; **before M1**, apply the two outstanding 02/01 edits above |

...
| M1 | `crates/futarchy-primitives` — `no_std` shared SCALE types + kernel `K` constants, `INTEGRATION_CONTRACT_VERSION = 2` (bump applied 2026-07-13 in doc 02, see Decision log) | 02 §2; 13 §2; 01 §5.2 | M0 | 🔨 | Initial crate/types/constants landed without `frame` deps; SCALE/metadata derive wiring has been added, but full verification is blocked by the container crates.io CONNECT 403 until registry access or vendored dependencies are available |

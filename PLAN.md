...
**Current: M2 — `crates/futarchy-fixed`**. The first workspace member for 64.64 LMSR math is in place with checked arithmetic, maker-adverse rounding helpers, stable two-outcome LMSR cost, and regression coverage for V1/V4; remaining work is replacing the temporary std `f64` transcendental kernel with the spec-required no_std deterministic approximation and MPFR corpus gate before M2 can be marked complete.
...
| M2 | `crates/futarchy-fixed` — 64.64 fixed point, exp2/log2/ln, maker-adverse rounding, error bounds | 04 §4 | M0 | 🔨 | Initial crate/API and vector tests landed; replace temporary std `f64` kernel with no_std deterministic ≤2 ulp implementation, then add MPFR corpus gate |

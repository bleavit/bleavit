...
**Current: A1 — `pallet-constitution`**. M3 is complete: `reference-model/` now provides an independent Python executable spec covering LMSR vectors/corpus generation, TWAP slew/mean behavior, ledger VOID/gate primitives, welfare helpers, decision reason-code scenarios, and treasury security-sizing arithmetic, with CI freshness checks for the generated vector corpus. Next is A1 (`pallet-constitution`).

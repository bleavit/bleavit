...
**Current: M2 — `crates/futarchy-fixed`**. The 64.64 LMSR math crate now builds its transcendental surface in `no_std` mode with deterministic in-crate `f64` range reduction/series helpers, checked arithmetic, maker-adverse rounding helpers, stable two-outcome LMSR cost, explicit buy/sell delta helpers, and regression coverage for V1/V4 plus the V6 domain edge; remaining work is replacing this interim deterministic kernel with the spec-required ≤2 ulp polynomial/integer implementation and adding the full MPFR-256 corpus gate before M2 can be marked complete.
...
| M2 | `crates/futarchy-fixed` — 64.64 fixed point, exp2/log2/ln, maker-adverse rounding, error bounds | 04 §4 | M0 | 🔨 | Interim no_std deterministic transcendental helpers landed; buy/sell delta helpers now reject states/trades past the 48·b domain edge; replace with the final ≤2 ulp polynomial/integer kernel, then add the full MPFR-256 corpus gate |

...
**Current: M2 — `crates/futarchy-fixed`**. The 64.64 LMSR math crate now builds its transcendental surface in `no_std` mode with deterministic in-crate `f64` range reduction/series helpers, checked arithmetic, maker-adverse rounding helpers, stable two-outcome LMSR cost, marginal price/displacement helpers, explicit buy/sell delta helpers, regression coverage for V1–V4 plus the V6 domain edge, and an interim generated high-precision LMSR corpus fixture/generator; the interim corpus now includes nearest 64.64 raw expectations and a CI freshness check; remaining work is replacing this interim deterministic kernel with the spec-required ≤2 ulp polynomial/integer implementation and scaling the corpus to the full MPFR-256 gate before M2 can be marked complete.
...
| M2 | `crates/futarchy-fixed` — 64.64 fixed point, exp2/log2/ln, maker-adverse rounding, error bounds | 04 §4 | M0 | 🔨 | Interim no_std deterministic transcendental helpers landed; price/displacement and buy/sell helpers now cover V1–V4 and reject states/trades past the 48·b domain edge; interim corpus has decimal + nearest-raw expectations and a CI freshness check; replace with the final ≤2 ulp polynomial/integer kernel, then scale the interim corpus fixture to the full MPFR-256 corpus gate |

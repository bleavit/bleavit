...
| `14-client-boot` | 15 §4.7/§4.8; 10 §3.1/§4.1/§4.3/§5.2 | **Boot leg passed end-to-end 2026-08-08 (F27, first execution).** The canonical client's own light client, started on this Node through `@bleavit/chain-client/node-light-client`: relay warp-sync, parachain linkage (*"Got parachain head from relay chain"*), a delivered finalized head, the parachain runtime downloaded and compiled, and 10 §5.2's classifier returning `classified` at **`spec_version` 2** — the value `SUPPORTED_RUNTIMES` commits descriptors for. Genesis hashes and bootnodes come from the spawned nodes, **never** from smoldot's `chainSpec_v1_genesisHash`, which is the value the identity check compares against. **The specs are the ones zombienet spawned** (`<netdir>/paseo-local.json`, `<netdir>/4242-paseo-local.json`): zombienet rewrites the relay spec at spawn, so the pristine generated spec has a different genesis and the client correctly refuses it | **The wrong-chain leg is red, and it found a client defect rather than a drill one** — see PLAN.md F27 and SQ-1026. `WrongChainError` is raised inside `getSmProvider`'s chain factory, which PAPI **retries indefinitely**, so the terminal `FE-BOOT-003` never reaches the caller: 265 raises in one run, none propagated, and `chain-session.ts`'s `instanceof WrongChainError` branch is unreachable. **Asset Hub** reports `unavailable`: para 1000 *does* produce blocks (observed at #130, finalizing), but its genesis is ~189k raw entries in a 79 MB spec and smoldot does not finish initialising it inside the leg's bound. That is a spec-size question, **not** drill 07's inherent trap — an earlier draft of this row said otherwise and was wrong |

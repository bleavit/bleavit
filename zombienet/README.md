...
| `10-client-integration` | 15 §4.7; 16 §2/§3/§11 | The standalone client runtime calls only `BleavitClient::ask`; the exact positional ingress reaches Bleavit and the uncalibrated hosted service refuses deterministically | none — the quickstart source is the drill source, checked byte-for-byte by `tools/ci/check-quickstart-drill.py` |
| `11-client-return-channel-absent` | 15 §4.7; 16 §12; I-36 | Drives register → open → seal → report publication with only client→Bleavit HRMP configured; asserts the authoritative report exists, no client push arrives, and the selected `Welfare::XcmTraffic` cell is unchanged | **Execution not yet witnessed**: every HRMP topology attempted so far stalls both parachains at block #0 on the pinned Cumulus DMQ-head mismatch; structural assertions remain runnable |
| `12-client-report-pull` | 15 §4.7; 16 §12; I-36 | Reads `QuestionService::Reports[0]` at a finalized header, obtains its trie proof, and compares the same `Welfare::XcmTraffic` cell before and after | the release-shaped refusal leg leaves the report absent until a governed service rate and admission fixture are supplied; a non-empty value is verified by the TypeScript kit |
| `13-client-ingress-negative-matrix` | 15 §4.7; 16 §12; 09 §6.5 | The harness-only raw sender submits the exact six-position shape with one of eight malformed mutations; each `MessageQueue.Processed` result is false | none — this sender is deliberately not part of the client integration API |
| `14-client-boot` | 15 §4.7/§4.8; 10 §3.1/§4.1/§4.3/§5.2; 11 §11.9; 02 §7.7 | **All four legs passed end-to-end 2026-08-08 (F18).** `pin` (6.0 s) reads each spawned chain's genesis hash and listen addresses, derives the Asset Hub genesis **state root** offline with `polkadot-parachain export-genesis-head`, and pins the trimmed spec. `boot` (345.9 s) starts the client's own light client through `@bleavit/chain-client/node-light-client`: relay warp-sync, parachain linkage, a delivered finalized head, the parachain runtime downloaded and compiled, and 10 §5.2's classifier returning `full` at **`spec_version` 2**. `funding` (17.4 s) opens 11 §11.9's two legs over the live chains — withdraw `ready` on the local chain alone at block 49, deposit `ready` with readers on **two** chains and the 02 §7.7 foreign verdict `wrong-chain`; the `ForeignAssets.Account` key it built matches `runtime/bleavit-runtime/fixtures/storage-keys.json` byte-for-byte, and `Constitution.PhaseFlags` decoded to `sudoPresent=true`. `wrong-chain` (1.9 s) refused a one-nibble-corrupted parachain pin as `FE-BOOT-003`, comparing the corrupted pin against the chain's real genesis — so SQ-1026 no longer reproduces. Genesis hashes and bootnodes come from the spawned nodes, **never** from smoldot's `chainSpec_v1_genesisHash`, which is the value the identity check compares against; the state root comes from the spawned **spec bytes**, so both sides of that check stay independent. **The specs are the ones zombienet spawned** (`<netdir>/paseo-local.json`, `<netdir>/4242-paseo-local.json`, `<netdir>/1000-paseo-local.json`): zombienet rewrites them at spawn, so the pristine generated spec is a different chain | **The deposit leg cannot reach mode `full` on any local topology, by construction.** 02 §7.7 pins the Asset Hub of the relay a release targets, and a locally generated Asset Hub can never carry that genesis — so the honest verdict here is `wrong-chain`, the deposit blocks with diagnostics, and the withdraw leg is unaffected, which is exactly what §7.7 and §11.9.2 require. The positive case needs a fork of the real Paseo Asset Hub. **Two real gaps the funding leg reported**: D-13's Phase-3 cap headroom has **no reader in `app/`**, so `depositBlocks` always blocks on *"the deposit caps could not be read"* during bootstrap; and the drill's account holds no USDC on either chain, so the balance rows block on real zeroes rather than on decode failures |

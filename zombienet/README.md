...
| `11-client-return-channel-absent` | 15 §4.7; 16 §12; I-36 | The same typed client ingress runs with only client→Bleavit HRMP configured, proving the topology does not require a return channel to submit | report push is intentionally unavailable; the egress path is not allowed to write welfare/XCM-health state |

...
Linked-worktree builds now resolve their source commit outside the isolated container rather than
mounting parent Git metadata. The OCI wrapper rejects dirty or moving sources, passes the validated
40-hex commit into the worker, and the worker records that explicit binding in build-info v3.

The SR-13 completion/retry path now reports the full bounded weight it executes. `RecoveryImages`
publishes an explicit unpin bound; production composes the generated preimage legacy-conversion and
heaviest-unrequest weights, while the guard adds its own recovery-commitment read/delete and completion
anchor delete. Exact failure/repaired regressions keep the dedicated halt sticky and prove repair does
not clear another live halt source.

The final closure pass removed caller-supplied evaluation callbacks from the transaction gate. Until
the owned metadata evaluator is complete, the only production behavior is a fresh-pin `FE-TX-004`
refusal; explicit-value proof construction and transition enumeration are test-only and blocked from
production by named dependency rules. Release monitoring now speaks the app's real release-document
contract and keeps the asset manifest M distinct from final manifest M′ throughout index production,
gateway comparison and `ReleaseChannel` binding.

The genuine artifact run then found that chain-spec generation replaced the canonical OCI primary
with a host rebuild, twice: once directly and once inside relay/drill generation. Both scripts now
take an explicit exact-Wasm input; the release workflow supplies the same OCI path to each, and the
ordinary/migration drill specs inherit it while fast-timing stays deliberately separate. A mocked
behavioral test preinstalls the pinned builder, makes any remaining Cargo invocation fatal and proves
both dev/local specs embed the input bytes, while workflow and environment contracts pin the second
handoff too.

The release job now also generates the N10 client-parachain spec before B7 evidence. The evidence
producer requires every chain spec declared by a selected topology, binds only an exact set of
primary-runtime spec names to the shipped Wasm, explicitly excludes the client and fast-timing test
runtimes from that comparison, and rejects unknown `bleavit-*` names. Regression coverage proves the
missing client spec blocks prerequisites and the second relay generator preserves exact primary
bytes across base, raw and migration outputs.
...
## Rebound the two app question-expiry tests to `plan/questions/`

Main CI run `31615004746` failed its App job after the plan split: the
transaction-builder expiry test still searched the removed spec-question table
inside `PLAN.md`, so it reported open item `SQ-564` as missing. The analysis
disclosure suite carried the same stale parser later in the app gate and would
have been the next failure.

Both suites now share `app/tests/shared/plan-questions.ts`, which reads the
canonical `status: open | resolved` enum from each
`plan/questions/<ID>.md` frontmatter block. The reader fails closed on malformed
ids, missing delimiters, duplicate status fields and unknown status values; the
existing open-item checks and the resolved `SQ-615` anti-vacuity leg exercise the
split layout directly. A spec-compliance review also rebound the two production
declaration comments those suites police; they no longer claim the removed
PLAN.md table owns expiry. No frontend behavior, specification, parameter or
milestone status changed.

Verification: app build and test TypeScript project green; all 8
transaction-builder test files and all 4 analysis test files green under direct
Node test globs; unreadable-obligation, plan-render and spec-question-batch gates
green. The repository's `run-suite.ts` wrapper could not observe child TAP output
in this sandbox and refused the run as an unknown test count, while the exact
Node globs it wraps reported 8/8 and 4/4; this is recorded as an environment
artifact rather than misreported as a gate pass.

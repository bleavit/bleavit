...
export function pipeline({ check = false }: PipelineOptions = {}) {
  ...
  // recipe as a CI one; the chain-side `tools/release/build-runtime-oci.sh` entry point and
  // its in-container worker put it in the build for the same reason. It does not make a clock-reading tool deterministic — only

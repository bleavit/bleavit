...
type IRuntimeCalls = {
    ...
    FutarchyApi: {
         ...
         */
        nav: RuntimeDescriptor<[], Anonymize<Ifpv42fvgi4b3>>;
        /**
...
export type Bleavit = {
    ...
    asset: IAsset;
    extensions: BleavitExtensions;
    getMetadata: () => Promise<Uint8Array>;
    genesis: string | undefined;
};
declare const _allDescriptors: Bleavit;
export default _allDescriptors;
export type BleavitApis = ApisFromDef<IRuntimeCalls>;
export type BleavitQueries = QueryFromPalletsDef<PalletsTypedef>;
export type BleavitCalls = TxFromPalletsDef<PalletsTypedef>;
export type BleavitEvents = EventsFromPalletsDef<PalletsTypedef>;
export type BleavitErrors = ErrorsFromPalletsDef<PalletsTypedef>;
export type BleavitConstants = ConstFromPalletsDef<PalletsTypedef>;
export type BleavitViewFns = ViewFnsFromPalletsDef<PalletsTypedef>;
export type BleavitCallData = Anonymize<Icscgdrls4bngd> & {
    value: {
        type: string;
    };
};
type AllInteractions = {
    storage: {
        ...
        Constitution: ['Params', 'CounterForParams', 'PhaseFlags', 'ReleaseChannel', 'Meters', 'Capabilities'];
        ConditionalLedger: ['Vaults', 'BaselineVaults', 'Positions', 'PositionCount', 'PositionTotals', 'DepositsHeld', 'TotalEscrowed', 'RedemptionFeesAccrued', 'LedgerDrifted', 'LastReconciliation', 'VaultTerminalAt', 'BaselineTerminalAt', 'SplitPausedUntil', 'FrozenUntil', 'FreezeRenewed'];
        Market: ['Markets', 'CounterForMarkets', 'ActiveMarketCount', 'ActiveExternalMarketCount', 'StoredExternalMarketCount', 'ExternalBookPairs', 'CounterForExternalBookPairs', 'MarketProtocolAccounts', 'CounterForMarketProtocolAccounts', 'ProposalMarketIds', 'BaselineMarketOf', 'SealedBaselineTwap', 'ClosedAt', 'SeededMarkets', 'SweptMarkets', 'NextMarketId', 'TwapCheckpoints', 'DecisionWindows', 'DecisionWindowOwners', 'RerunSeededMarkets', 'SettlementObservedAt', 'LivePolCommitments', 'CreationFrozenUntil', 'FrozenUntil', 'FreezeRenewed'];
        Welfare: ['MetricSpecs', 'Snapshots', 'SnapshotContexts', 'SnapshotDeadline', 'GateBreachFlags', 'SampledGateDays', 'XcmTraffic', 'XcmTrafficEpochs', 'ReserveProbeDaily', 'CollatorAuthorship', 'CollatorAuthorshipEpoch', 'BlockProduction', 'BlockProductionEpoch', 'BlockWeightSamples', 'PrimaryBlockWeightSamples', 'BlockResourceUsage', 'IntegrityFailures'];
        Oracle: ['Reporters', 'CounterForReporters', 'Watchtowers', 'CounterForWatchtowers', 'Rounds', 'RoundSchedules', 'ComponentValues', 'ReserveHealth', 'ReserveProbeArmed', 'ReserveProbeArmedAt', 'AckRecords', 'WatchtowerActive', 'MoneySettled', 'ReporterRecords', 'RoundActivity', 'Recomputable'];
        IncidentRegistry: ['Filings', 'FilingCount', 'Aggregates', 'AckRecords', 'ClosedAt'];
        MilestoneRegistry: ['Filings', 'FilingCount', 'Aggregates', 'AckRecords', 'ClosedAt'];
        FutarchyTreasury: ['State', 'PendingMainCredit', 'SweptResidueUnreclaimed', 'CoretimeQuoteAuthority', 'BootstrapOpsFundingClosed', 'CommunityDistributionArmedAt', 'CommunityDistributionRemaining', 'CommunityScheduleCount', 'CoretimeRenewalAccount', 'CollatorAuthoredBlocks', 'CollatorAuthoredEpoch', 'CollatorAuthoredRegisteredCount', 'CollatorPendingBlocks', 'CollatorPendingEpoch', 'CollatorPendingRegisteredCount', 'CollatorAuthoredOverflowed', 'CollatorPendingOverflowed', 'CollatorDroppedEpoch', 'CollatorCompensationPaidEpoch'];
        Guardian: ['Members', 'MemberBonds', 'PendingActions', 'Approvals', 'ReviewDeadlines', 'ActivePlaybooks', 'PlaybookRegistered', 'RerunUsed', 'Allowances', 'NextActionId', 'LastSeenEpoch', 'ReviewReferenda', 'VetoReviewReferenda', 'CounterForVetoReviewReferenda', 'VetoReviewActions', 'CounterForVetoReviewActions', 'ReviewFrontingOf', 'CounterForReviewFrontingOf', 'PendingBondReleases', 'FailedActions', 'CounterForFailedActions', 'MaintenanceSweepCursor', 'FailedActionReapCursor'];
        Attestor: ['Members', 'Attestations', 'Liabilities', 'Revocations', 'NextAttestationId'];
    ...
    tx: {
        ...
        Migrations: ['force_set_cursor', 'force_set_active_cursor', 'force_onboard_mbms', 'clear_historic'];
        ...
        Constitution: ['set_param', 'set_capability', 'set_phase_flag', 'set_release_channel', 'amend_registry'];
        ConditionalLedger: ['split', 'merge', 'split_scalar', 'merge_scalar', 'split_gate', 'merge_gate', 'transfer', 'split_baseline', 'merge_baseline', 'resolve', 'void', 'settle_scalar', 'settle_gate', 'settle_baseline', 'redeem', 'redeem_scalar', 'redeem_scalar_pair', 'redeem_gate', 'redeem_void', 'redeem_baseline', 'redeem_baseline_pair', 'sweep_dust', 'sweep_dust_baseline', 'set_split_paused', 'set_frozen', 'reconcile', 'sweep_redemption_fees'];
        Market: ['buy', 'sell', 'crank_observe', 'sweep_revenue', 'reap', 'freeze_creation', 'set_frozen'];
        Welfare: ['register_spec', 'record_snapshot', 'record_daily_gate'];
        Oracle: ['register_reporter', 'deregister_reporter', 'report', 'challenge', 'counter_report', 'recompute_proof', 'register_watchtower', 'ack_observed', 'crank_round_close', 'crank_reserve_probe', 'adjudicate'];
        IncidentRegistry: ['file', 'challenge_filing', 'ack_observed', 'crank_close', 'resolve_challenge', 'close_epoch', 'reap_epoch'];
        MilestoneRegistry: ['file', 'challenge_filing', 'ack_observed', 'crank_close', 'resolve_challenge', 'close_epoch', 'reap_epoch'];
        FutarchyTreasury: ['fund_budget_line', 'spend', 'open_stream', 'claim_stream', 'cancel_stream', 'issue_vit', 'recover_foreign', 'execute_coretime_renewal', 'note_coretime_quote', 'prune_coretime_quote', 'set_coretime_authority', 'sweep_insurance', 'reconcile_insurance', 'create_community_schedule'];
        Guardian: ['set_members', 'propose_action', 'approve_action', 'ratify_action', 'renew_playbook', 'uphold_veto', 'recall', 'set_playbook_registered'];
        Attestor: ['set_members', 'attest', 'challenge_attestation', 'resolve_challenge', 'remove_for_cause', 'reap_attestation'];
        Epoch: ['submit', 'withdraw', 'tick', 'decide', 'settle_cohort', 'set_next_epoch_length', 'delay_once', 'mark_executed', 'mark_failed_executed', 'retry_exhausted_to_measurement', 'expire_or_stale_queue', 'force_reject_process_hold', 'void_cohort', 'set_intake_paused', 'finalize_epoch_baseline', 'drive_oracle_boundaries', 'bind_ratification'];
        ExecutionGuard: ['execute', 'apply_authorized_upgrade', 'expire_failed_execution', 'ratify', 'reject_stale', 'commit_recovery_image', 'authorize_phase_four', 'qualify_recovery_image'];
        ClientRegistry: ['admit_client', 'admit_local_client', 'remove_client', 'top_up_delivery_float', 'withdraw_delivery_float'];
        QuestionService: ['register', 'bond_attestor', 'open', 'seal', 'submit_attestation', 'settle', 'void', 'set_paused', 'archive'];
        ServiceLedger: ['split', 'merge', 'split_scalar', 'merge_scalar', 'split_gate', 'merge_gate', 'transfer', 'split_baseline', 'merge_baseline', 'resolve', 'void', 'settle_scalar', 'settle_gate', 'settle_baseline', 'redeem', 'redeem_scalar', 'redeem_scalar_pair', 'redeem_gate', 'redeem_void', 'redeem_baseline', 'redeem_baseline_pair', 'sweep_dust', 'sweep_dust_baseline', 'set_split_paused', 'set_frozen', 'reconcile', 'sweep_redemption_fees'];
    };
    events: {
        ...
        Migrations: ['UpgradeStarted', 'UpgradeCompleted', 'UpgradeFailed', 'MigrationSkipped', 'MigrationAdvanced', 'MigrationCompleted', 'MigrationFailed', 'HistoricCleared'];
        ...
        Constitution: ['ParamUpdated', 'CapabilitySet', 'PhaseFlagSet', 'ReleaseChannelSet', 'RegistryAmended', 'MeterCharged'];
        ConditionalLedger: ['Split', 'Merged', 'ScalarSplit', 'ScalarMerged', 'GateSplit', 'GateMerged', 'PositionTransferred', 'BaselineSplit', 'BaselineMerged', 'VaultResolved', 'VaultVoided', 'ScalarSettlementSet', 'GateSettled', 'BaselineSettled', 'Redeemed', 'ScalarRedeemed', 'ScalarPairRedeemed', 'GateRedeemed', 'VoidRedeemed', 'BaselineRedeemed', 'RedemptionFeesSwept', 'VaultReaped', 'BaselineVaultReaped', 'SplitPauseSet', 'SplitPauseCleared', 'FreezeSet', 'FreezeCleared', 'FreezeExtended', 'LedgerDriftDetected', 'LedgerDriftCleared'];
        Market: ['Traded', 'Observed', 'MarketCreated', 'MarketClosed', 'MarketReaped', 'Seeded', 'CreationFreezeSet', 'CreationFreezeCleared', 'FreezeSet', 'FreezeCleared', 'FreezeExtended', 'RevenueSwept', 'ExternalRevenueSwept'];
        Welfare: ['MetricSpecRegistered', 'SnapshotRecorded', 'GateBreachRecorded', 'SettlementComputed', 'SettlementRenormalized', 'IntegrityFailureRecorded'];
        Oracle: ['ReporterRegistered', 'Reported', 'Challenged', 'RoundEscalated', 'RecomputeProven', 'AdjudicationRequested', 'Adjudicated', 'ComponentSettled', 'NeutralSettlement', 'WindowAcknowledged', 'WindowExtended', 'QuorumFailed', 'ReporterSlashed', 'ReporterEjected', 'WatchtowerRegistered', 'WatchtowerInactive', 'WatchtowerSlashed', 'ReserveProbeSent', 'ReserveProbeResult', 'ReserveUnhealthy', 'ReserveRecovered', 'RetentionExpired', 'ReporterRecordsFull'];
        IncidentRegistry: ['IncidentFiled', 'MilestoneFiled', 'IncidentChallenged', 'MilestoneChallenged', 'IncidentUpheld', 'IncidentRejected', 'MilestoneAccepted', 'MilestoneRejected', 'FilingBondSlashed', 'RegistryEpochClosed', 'WindowAcknowledged', 'WindowExtended'];
        MilestoneRegistry: ['IncidentFiled', 'MilestoneFiled', 'IncidentChallenged', 'MilestoneChallenged', 'IncidentUpheld', 'IncidentRejected', 'MilestoneAccepted', 'MilestoneRejected', 'FilingBondSlashed', 'RegistryEpochClosed', 'WindowAcknowledged', 'WindowExtended'];
        ...
        Guardian: ['GuardianAction', 'ForceRerun', 'PlaybookActivated', 'PlaybookRenewed', 'PlaybookExpired', 'ReviewScheduled', 'MembersSet', 'ActionProposed', 'ActionApproved', 'ActionRatified', 'ReviewFailed', 'RecallScheduled', 'RecallEnacted', 'PlaybookRegistrationSet'];
        Attestor: ['MembersSet', 'AttestationSubmitted', 'AttestationChallenged', 'ChallengeResolved', 'AttestorEjected', 'AttestorRemovedForCause', 'AttestationRevoked'];
        Epoch: ['ProposalSubmitted', 'ProposalWithdrawn', 'ScreeningStarted', 'ProposalCancelled', 'ProposalQualified', 'ProposalDeferred', 'SlotsShrunk', 'MarketsOpened', 'DecisionExtended', 'ProposalQueued', 'ProposalRejected', 'ProposalDelayed', 'RerunScheduled', 'RerunOpened', 'MandateExpired', 'MeasurementStarted', 'CohortSettled', 'CohortVoided', 'BaselineCarried', 'ProposalForceRejected', 'IntakeSlashed', 'IntakePauseSet', 'IntakePauseCleared', 'OracleDeadlockLatched', 'OracleDeadlockCleared'];
        ExecutionGuard: ['Executed', 'ExecutionFailed', 'Ratified', 'UpgradeAuthorized', 'Enqueued', 'Rejected', 'UpgradeApplied', 'PreimageUnpinned', 'UpgradeAborted', 'PendingOutflowSyncFailed', 'MigrationHalted', 'RecoveryImageCommitted', 'RecoveryImageApplied', 'PhaseFourUpgradeAuthorized', 'RecoveryImageQualified'];
        ClientRegistry: ['ClientAdmitted', 'LocalClientAdmitted', 'ClientRemovalStarted', 'ClientRemoved', 'EgressPrepaid', 'DeliveryFloatToppedUp', 'DeliveryFloatWithdrawn'];
        QuestionService: ['QuestionRegistered', 'QuestionSealed', 'QuestionSettled', 'QuestionVoided', 'AttestorBonded', 'AttestationSubmitted', 'ServicePauseSet', 'ServicePauseCleared', 'QuestionArchived'];
    ...
    constants: {
        ...
        Migrations: ['CursorMaxLen', 'IdentifierMaxLen'];
        ...
        Session: ['KeyDeposit'];
        Aura: ['SlotDuration'];
        Constitution: ['INTEGRATION_CONTRACT_VERSION', 'MaxParams', 'MaxCapabilities', 'MaxMeters'];
        ConditionalLedger: ['MinSplit', 'PositionDeposit', 'MaxPositionsPerAccount', 'ArchiveDelay', 'ReapBatch', 'PalletId', 'MinTransfer', 'RedemptionFee', 'ServiceIdBase'];
        Market: ['Fee', 'ObsInterval', 'Kappa1e9', 'ArchiveDelay', 'PalletId', 'MinTrade', 'MaxTradeRatio', 'MaxLiveMarkets', 'MaxStoredMarkets', 'MaxLiveExternalMarkets', 'MaxStoredExternalMarkets', 'MaxAllStoredMarkets', 'GatePMaxCeiling', 'GateEpsFloor'];
        Welfare: ['INTEGRATION_CONTRACT_VERSION', 'MaxMetricSpecs', 'MaxSnapshots', 'MaxGateFlags', 'MaxDailyGateSamples'];
        Oracle: ['MaxRoundCloseBatch'];
        IncidentRegistry: ['Kind', 'PalletId', 'ArchiveDelay', 'MaxFilingsPerEpoch', 'MaxEvidenceLen'];
        MilestoneRegistry: ['Kind', 'PalletId', 'ArchiveDelay', 'MaxFilingsPerEpoch', 'MaxEvidenceLen'];
        FutarchyTreasury: ['INTEGRATION_CONTRACT_VERSION', 'MaxStreams', 'MaxBudgetLines', 'MaxPolCommitments', 'MaxCollatorCompensationEntries'];
        Guardian: ['GuardianSeats', 'GuardianThreshold', 'GuardianBond', 'PlaybookFreezeWindowBlocks'];
        Attestor: ['AttMinMembers', 'AttQuorum', 'ChallengeWindowBlocks'];
        Epoch: ['INTEGRATION_CONTRACT_VERSION', 'TreasuryBondAskBps', 'MaxLiveProposals', 'MaxIntakeQueue', 'MaxNonTerminalCohorts', 'RecentCohortSummariesBound', 'TickBatch', 'PhaseOffsets', 'MaxBooksPerProposal', 'MinEpochLength', 'DecisionWindowFloor', 'DecisionExtension', 'DecisionDeltaFloors', 'DecisionSigmaFloors'];
        ExecutionGuard: ['MaxRuntimeCodeBytes', 'INTEGRATION_CONTRACT_VERSION', 'MaxLiveProposals', 'MaxExecutionRecords', 'MaxCalls', 'MaxPayloadBytes', 'DescriptorLeadTime', 'MaxRuntimeCodeBytes', 'ExecutionTimelockFloor', 'ExecutionGraceFloor'];
        ClientRegistry: ['DeliveryAssetId', 'DeliveryFloatPalletId', 'MaxClients', 'ClientBond'];
        QuestionService: ['PalletId', 'FeeFloor', 'MaxLive', 'MaxWindow', 'EpsilonMin', 'AttestorsMin'];
        ServiceLedger: ['MinSplit', 'PositionDeposit', 'MaxPositionsPerAccount', 'ArchiveDelay', 'ReapBatch', 'PalletId', 'MinTransfer', 'RedemptionFee', 'ServiceIdBase'];
    };
    viewFns: {};
    apis: {
        ...
        FutarchyApi: ['epoch_status', 'proposal_summaries', 'quote', 'decision_stats', 'account_positions', 'execution_queue', 'welfare_current', 'params', 'nav', 'recent_cohorts', 'open_oracle_rounds', 'hosted_report', 'service_positions', 'is_reserved_protocol_destination'];
        ...
        KeyToIncludeInRelayProof: ['keys_to_prove'];
...
export type BleavitWhitelistEntry = PalletKey | `query.${NestedKey<AllInteractions['storage']>}` | `tx.${NestedKey<AllInteractions['tx']>}` | `event.${NestedKey<AllInteractions['events']>}` | `error.${NestedKey<AllInteractions['errors']>}` | `const.${NestedKey<AllInteractions['constants']>}` | `view.${NestedKey<AllInteractions['viewFns']>}` | `api.${NestedKey<AllInteractions['apis']>}`;
type PalletKey = `*.${({

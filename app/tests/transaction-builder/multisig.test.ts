...
const blake2b256 = (bytes) => blake2b(bytes, { dkLen: 32 });
...
test('the SCALE compact length widens at 64 signatories', () => {
  ...
  assert.equal(a.preimage[16] & 0b11, 0b00, 'single-byte compact mode');
  assert.equal(b.preimage[16] & 0b11, 0b01, 'two-byte compact mode');
});
...
test('the caller need not sort, and the order it supplies changes nothing', () => {
  ...
  const shuffled = [c.signatories[2], c.signatories[0], c.signatories[1]];
...
test('uppercase hex is accepted and normalised, not silently mis-sorted', () => {
  ...
  // different account.
  const c = caseNamed('two_of_three');
  const upper = c.signatories.map((k) => '0x' + k.slice(2).toUpperCase());
...
test('a duplicate signatory is refused rather than deduplicated', () => {
  ...
  const c = caseNamed('two_of_three');
  assert.throws(
    () => deriveMultisigAccount([...c.signatories, c.signatories[0]], 2, blake2b256),
    MultisigDerivationError,
  );
});
...
test('a malformed public key is refused, not padded', () => {
  const c = caseNamed('two_of_three');
  for (const bad of ['0x00', 'not-hex', '', '0x' + 'g'.repeat(64), c.signatories[0].slice(2)]) {
    assert.throws(
      () => deriveMultisigAccount([bad, c.signatories[1]], 1, blake2b256),
      MultisigDerivationError,
      JSON.stringify(bad),
    );
  }
...
test('the hash is required and its output width is checked', () => {
  ...
  const c = caseNamed('two_of_three');
  assert.throws(() => deriveMultisigAccount(c.signatories, 2, undefined), MultisigDerivationError);
  assert.throws(
    () => deriveMultisigAccount(c.signatories, 2, (b) => blake2b(b, { dkLen: 64 })),
    MultisigDerivationError,
  );
});
...
test('other_signatories excludes the signer and keeps the ascending order', () => {
  const c = caseNamed('two_of_three');
  const derived = deriveMultisigAccount(c.signatories, c.threshold, blake2b256);
  const others = otherSignatories(derived, c.signatories[1]);
  assert.deepEqual([...others], [c.signatories[0], c.signatories[2]]);
});

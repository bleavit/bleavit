...
| `crates/bleavit-client-abi/` | N9 `no_std` wire ABI for the one fixed hosted-report receiver call (`[66, 0] ++ SCALE(ReportView)`), shared by Bleavit egress and N10's drop-in client receiver |
...
| `pallets/`, `crates/*-core/` | Track A (complete) plus Track N through N9: each `crates/<name>-core/` is a frame-free functional core and each matching `pallets/<name>/` its production `#[frame_support::pallet]` shell. N4 adds `client-registry-core` / `pallet-client-registry` with exact transport identity, native bond custody and `ExternalClient(ClientId)`; N7 adds `pallet-question-service` over the N5 service core; N8 wires that identity to the XCM executor; N9 adds separate client-funded USDC delivery custody and best-effort report egress without another pallet |
...
| `runtime-api/` | `futarchy-runtime-api`: the `sp_api::decl_runtime_apis!` declaration of the frozen 12-method contract-v22 `FutarchyApi` (02 §3–§4a) over the view types in `futarchy-primitives`; `hosted_report` remains unchanged, while the monitoring-only API v4 adds isolated per-client egress counters outside the integration contract |

...
| `pallets/`, `crates/*-core/` | Track A (complete) plus Track N through N7: each `crates/<name>-core/` is a frame-free functional core and each matching `pallets/<name>/` its production `#[frame_support::pallet]` shell. N4 adds `client-registry-core` / `pallet-client-registry` with exact transport identity, native bond custody and `ExternalClient(ClientId)`; N7 adds `pallet-question-service` over the N5 service core |

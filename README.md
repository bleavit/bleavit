...
| `pallets/`, `crates/*-core/` | Track A (complete) plus Track N through N8: each `crates/<name>-core/` is a frame-free functional core and each matching `pallets/<name>/` its production `#[frame_support::pallet]` shell. N4 adds `client-registry-core` / `pallet-client-registry` with exact transport identity, native bond custody and `ExternalClient(ClientId)`; N7 adds `pallet-question-service` over the N5 service core; N8 wires that identity to the XCM executor without adding another pallet |

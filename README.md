...
| `runtime/bleavit-runtime/` | B1a + B6 + review-open B1b: the real Cumulus parachain runtime — `construct_runtime!` over the Track-A + standard/system pallets (`Epoch` at frozen index 61, `ExecutionGuard` at 62), real epoch clock/market/I-9 wiring, `BaseCallFilter = SafetyFilter`, the full 09 §2 upgrade path, genesis presets carrying the 08 §2.1 VIT allocation/vesting, and canonical release/benchmark Wasm builds. B1b remains 🔨 for PLAN SQ-140…SQ-150. |

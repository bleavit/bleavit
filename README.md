...
| `pallets/`, `crates/*-core/` | Track A (complete) plus N4: each `crates/<name>-core/` is a frame-free functional core and each matching `pallets/<name>/` its production `#[frame_support::pallet]` shell. N4 adds `client-registry-core` / `pallet-client-registry` with exact-`Location` identity, native bond custody and `ExternalClient(ClientId)` |

...
2. Resolve and fetch by both canonical name and immutable TXID. Parse the
   canonical `bleavit.app-release.v1` document, fetch the asset manifest **M**
   named by `arweaveManifestTxId`, and require its paths and served bytes to match
   `perFileHashes`. Require the final manifest **M′** to differ from M, list exactly
   M's paths plus `release.json`, and match every canonical-name and immutable
   route; each root/default route must equal M′'s verified `index.path` bytes.
   ...
   hash. Require its `release_json_sha256` to match the served bytes and its
   `manifest_txid` (M′) to match every resolver and `ReleaseChannel`; only then
   use its transaction IDs to verify minisign

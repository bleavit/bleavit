---
name: implement
description: Run spec-driven implementation increments for the Bleavit futarchy system — pick (or receive) the next plan-tree milestone, implement it strictly from docs/architecture/, verify it per doc 15, update the plan tree, then continue to the next item. Use at the start of a working session, when the user says "continue" / "next step" / "weiter", or when a specific milestone is named.
...
- Read `PLAN.md`, the selected `plan/milestones/<ID>.md`, relevant open
  `plan/questions/` items, and the newest `plan/log/` records.
...
- Read every `docs/architecture/` section cited in the milestone's `spec:` field,
...
- The spec is the source of truth (rule R-1). If it is ambiguous, contradictory, or
  silent on something you need: create a `plan/questions/SQ-*.md` item (cite doc §),
  ...
  logged in `plan/decisions/`) — but confirm non-obvious semantic changes with the user
  first rather than coding around or silently rewriting the spec.

## 3. Declare scope

- Set the milestone's `status:` to `active` and write one sentence in **Current focus**:
...
- Write/extend the tests the milestone's `verify:` field and doc 15 require —
...
- Launch the `spec-reviewer` subagent on the changed component. Fix blockers/majors;
  minors may be logged as follow-ups in the milestone body.

## 6. Close the session

- Update the milestone item: `status: done` only if gates are green and the
  spec-reviewer found no blockers; otherwise keep `active` with precise resume notes.
  Update Current focus when it changes and append today's `plan/log/` day file.
...
- Never resolve a `[VERIFY]` tag by assumption: verify against live sources
  (WebSearch/WebFetch) and record the outcome in `plan/verifications/V-*.md`.
- If context is running low mid-milestone, stop at a clean boundary, write exact
  resume notes in the milestone and session-log files, and end gracefully.

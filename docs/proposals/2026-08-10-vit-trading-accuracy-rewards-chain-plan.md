...
**And one from TR3, which you should decide rather than inherit.** TR3's I-2 fix retains a participant record at `bond = 0` while an accrual is unclaimed, so a bond is never hostage to the VIT budget. `is_enrolled` is still a bare `contains_key`, which means **TR4 will score fills for a retained zero-bond record**. Its cap is 0, so every outcome is `Neutral` and no reward can arise — the only cost is score rows that exist for nothing. TR3 deliberately did not narrow the signature, because `is_enrolled` is your contract, not its. Either narrow it here or state why scoring a zero-cap record is acceptable.

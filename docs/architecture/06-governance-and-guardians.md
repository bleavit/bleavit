...
| -------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------------------ | ---------------------------------- | ------------------------------------------------------ |
| `metric`       | ConstitutionalValues | `welfare.register_spec`, `welfare.activate_spec`, metric weights within kernel bounds                                                          | 10,000 WIT | 2 d / 14 d / 2 d         | linear 60%→50% / reciprocal 10%→2% | 14 d (+ activation ≥ 2 epochs)                         |
| `constitution` | ConstitutionalValues | `constitution.amend_registry` within meta-bounds; floor tightening                                                                             | 25,000 WIT | 2 d / 21 d / 3 d         | 67% / 15%→5%                       | 28 d                                                   |
| `entrenched`   | ConstitutionalValues | floor loosening; guardian-scope change; kernel-adjacent registry entries                                                                       | 50,000 WIT | 7 d / 28 d / 7 d         | **80% / 20%→10%**                  | 4 epochs                                               |
| `guardian`     | ConstitutionalValues | `guardian.set_members`, recall, playbook-registry ratification, **`attestor.set_members` / attestor recall (§7), `guardian.renew_playbook` (§6.3)** | 5,000 WIT  | 1 d / 7 d / 1 d          | 55% / 5%                           | 2 d                                                    |
| `ratify`       | ConstitutionalValues | **`execution_guard.ratify(proposal_id, referendum_index)`** (§2.4); **`guardian.ratify_action(action_id)`** (§5.4)                             | 1,000 WIT  | 1 d / 7 d / 1 d          | 50% / 5%                           | **immediate — the only deadline is at `execute()` (§2.2)** |
| `oracle`       | OracleResolution     | `oracle.adjudicate(round_id, verdict)` only                                                                                                    | 5,000 WIT  | **0 / 7 d / 1 d**        | **60% / 10%→3%**                   | immediate                                              |
...
2. **Pre-cohort conviction snapshot.** The tally for an `OracleResolution` referendum concerning a dispute whose subject is cohort *e* weights each vote by `min(current conviction-locked WIT, conviction-locked WIT as of SnapshotBlock(e))`, where `SnapshotBlock(e)` is the block at which cohort *e* was created (recorded by `pallet-epoch` as `CohortSnapshotBlock: map EpochId → BlockNumber`, bounded by the ≤ 4 live cohorts). **Capital that entered, or locked, after the disputed cohort began therefore has zero incremental weight in its adjudication** — buying WIT after seeing the dispute cannot buy the verdict. Locks reduced or unlocked since the snapshot count at their current (lower) value, so exiting always costs weight and never adds it.
...
| `att.bond`                           | 25,000 WIT                           | entrenched              |

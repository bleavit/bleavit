...
**Rows without a seeded `Params` record (normative; amended 2026-07-20, SQ-158).** Not every §1 row materializes as a genesis-seeded record. A row whose class is `entrenched` with a kernel-scoped bound and neither a max-Δ nor a cooldown — `grd.bond` / allowances — is carried as a **kernel constant exposed through pallet metadata** rather than as a `Params` record: it has no amendment path by construction, so a record carrying governance metadata would misdescribe it. Reading rule 3 is satisfied for such a row by the constants API, not by a storage item.

**Unbounded sentinel (normative; amended 2026-07-20, SQ-196).** The Phase-5+ "unbounded sentinels" of [09 §5.2](09-execution-upgrades-and-rollout.md) are the **maximum representable value of the key's `Balance` type**, which is also the seeded hard max of `phase3.tvl_cap` and `phase3.dep_cap` — so arming one is an ordinary amendment of the row to its own ceiling, requiring no distinguished value and no separate mechanism. Installing it **retires** the containment meter rather than merely widening it: while a cap reads as the sentinel the meter records nothing — it neither reads nor writes the per-account cumulative — and every inflow is admissible. Entries already recorded are **retained, never reaped**; they stay visible to `try-state` and bind again unchanged if a bounded cap is later restored.

...
2. **Arithmetic, exactly as the runtime performs it.** `fee(g) = 0` if `g < MinSplit`, else `ceil(g · ledger.redeem_fee)`; `net = g − fee(g)` ([03](03-conditional-ledger.md) §5.3a(2)). The fee rounds **up**, against the claimant — the client MUST reproduce that direction and MUST NOT round to a friendlier figure, or the confirm screen becomes the optimistic estimate the precondition system exists to eliminate.

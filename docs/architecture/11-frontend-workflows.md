...
| S4 | Positions / transfer / redeem (incl. **Voided**), **both ledger domains** | core | `Positions` prefix / `account_positions()`, **`ServiceLedger.Positions` prefix / `service_positions()`**, `Vaults(pid)`, `BaselineVaults(epoch)` | `ledger.split/merge/split_scalar/merge_scalar/split_gate/merge_gate/transfer/redeem/redeem_scalar/redeem_scalar_pair/redeem_gate/redeem_baseline/redeem_baseline_pair/redeem_void` | §11.5, §11.6, §11.2a |
...
Four rules bound how they are served, and they exist because the two domains are genuinely different, not merely differently named:

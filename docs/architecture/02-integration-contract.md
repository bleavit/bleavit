...
**Ownership and freeze (D-2, resolves F-4).** This contract is **jointly owned by the backend and frontend teams**. It is frozen at **contract version 24**; the hosted-question/report surface, client-paid egress, the canonical client's external-book reads and the §7.6 client-read surfaces are in force (§4a/§7.6/§13). Any change — additive or otherwise — REQUIRES sign-off from both teams and a version bump (§13). The contingency for contract breach is the D-6 layer-1 fallback (chain-served ring + TWAP checkpoints), never a third-party service.
...
| 2 | `FutarchyApi` — 13 runtime-API methods + view types | §3–§4a | FE §30 P-5/P-7 + D-20, completed |
...
### 7.6 SDK pallet storage the frontend reads (contract v24; SQ-580)
...
This is not a new category. §7.4 already freezes `System.Account` and `ForeignAssets.Account(USDC_LOCATION, who)` on exactly this basis; v24 completes the set against what [10](10-frontend-architecture.md) and [11](11-frontend-workflows.md) actually mandate.
...
| Contract version | `INTEGRATION_CONTRACT_VERSION` (runtime constant) — **`24` in force** (§13's history is the authority; read the constant, never a prose copy of it) |

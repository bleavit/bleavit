...
G-3. **Annulment.** On **normal losing-branch resolution**, trades in the unrealized branch are economically reverted for the dominant user path: buyers hold the mirror branch-USDC (D-3) and redeem `cost` at par, losing only fees. Holders complete through **both** ledger layers (Accept *and* Reject branch-USDC) always recover par, including under VOID (D-1); same-branch LONG+SHORT completeness alone does not, since merging it yields one same-branch branch-USDC worth 0.5 under VOID. Under **protocol VOID** a wrapper buyer's whole package instead recovers its D-1 **neutral value** after pair-first netting — equal to the debit only when the realized average execution price is 0.5 — and a deliberately unpaired holder receives that same neutral valuation. No claim is ever haircut below the D-1 schedule, but VOID does not refund a premium paid over the neutral prior (SQ-171). **This guarantee is unchanged by the E1 redemption fee, and deliberately so** (normative; added 2026-07-29, milestone E1): [03](03-conditional-ledger.md) §5.3a charges `ledger.redeem_fee` on **settlement payouts only** and exempts exactly the legs G-3 rests on — `redeem` (the par leg), `redeem_void`, every `merge*` primitive, every `ProtocolAccount`, and any gross payout below `ledger.min_split`. The mirror credit is therefore still redeemed at par, "losing only fees" still means **trade fees alone**, and the cross-branch complete-pair par recovery under VOID is untouched. The scoping is a property of the design, not an omission: widening the charged set into `redeem` or `merge` would falsify this guarantee together with D-3, I-2(b), I-5 and PT-2, and is a **breaking change to G-3**, not a parameter move ([00](00-decision-record.md) D-3).
...
| `pallet-transaction-payment` + `pallet-asset-tx-payment` | Fees in VIT or USDC | Conversion bound to constitution key **`fee.vit_usdc_rate`** (typed, bounded, PARAM-adjustable — D-12, [08](08-treasury-and-economics.md)); USDC-only users can always pay fees in USDC. **Destination (E1, D-12): collected USDC routes to the treasury `MAIN` account and enters NAV at par — a handler that drops it is non-conforming — while collected VIT continues to burn; the asymmetry is the bridged/native distinction ([08 §9](08-treasury-and-economics.md))** |
...
| `pallet-conditional-ledger` | Conditional custody: split/merge/scalar ops/transfer/resolve/redeem/settle; **`Voided` state + `redeem_void` (D-1)**; **gate instruments (B-2)**; **epoch-keyed `BaselineVaults` (B-3)**; per-branch supplies and per-branch conservation identity (B-4); `redeem_scalar_pair` (B-5); **the `ledger.redeem_fee` redemption fee on settlement payouts, retained as sovereign surplus and remitted by the permissionless `sweep_redemption_fees()` crank — with `redeem`, `redeem_void` and every `merge*` exempt (E1, [03 §5.3a](03-conditional-ledger.md))** | `pallet-assets` (USDC) | [03](03-conditional-ledger.md) |
| `pallet-market` | LMSR books (branch-USDC denominated, D-3), trading wrapper, fees, TWAP, POL seeding; **realized fee value routes 100 % to the treasury `MAIN` account and POL custody returns to `POL`/`POL_BASELINE`, both through the permissionless `sweep_revenue(market)` crank that gates reap (E1, [04 §2/§6.1](04-markets-and-pricing.md); supersedes the 50 % `INSURANCE` / 50 % POL-offset split)**; **`Traded`/`Observed` events ([02 §6](02-integration-contract.md))**; **`BaselineMarketOf` map ([02 §7](02-integration-contract.md), [04 §8.3](04-markets-and-pricing.md))**; books/proposal ≤ 6, with a 196-book active/POL envelope and independent 2,240-row retained-book envelope shared by proposal and Baseline books *(normative values: [13](13-parameters.md))* | ledger, constitution | [04](04-markets-and-pricing.md) |
...
| `pallet-futarchy-treasury` | Treasury accounts, NAV with haircuts, rate-limited class-gated outflows, streams, budget lines (incl. bootnode/RPC/keeper/oracle-evidence/ArNS ops lines, D-16); **receives all protocol revenue in `MAIN` — market-fee, redemption-fee and USDC transaction-fee value — and holds `INSURANCE` to a derived target above which the surplus overflows to `MAIN` automatically, `sweep_insurance` remaining the only path below it (E1, [08 §1.1/§1.2](08-treasury-and-economics.md))** | assets, constitution | [08](08-treasury-and-economics.md) |
...
- **Frontend stack** — verified against `registry.npmjs.org` and the primary GitHub repos: `polkadot-api` 2.1.8, `smoldot` 3.3.1, `@substrate/connect` 2.1.8, `vite` 8.1.4, `dexie` 4.4.4, `idb` 8.0.3, `@ar.io/wayfinder-core` 2.0.0, `@ar.io/wayfinder-react` 2.0.1, `@ar.io/sdk` 4.0.3, `permaweb-deploy` 5.0.0, `@ardrive/turbo-sdk` 1.42.0 (`arkb` last published 2022 — rejected as unmaintained; `@reactive-dot/react` 0.72.0 evaluated, not adopted). **Re-verified as this clause requires: the shipped smoldot pin is `3.3.2`, not the 3.3.1 recorded above** — `app/pnpm-lock.yaml` resolves it transitively through `@polkadot-api/smoldot@0.4.6`, and every FE-P2/FE-P5 finding was read from that version's source at the tag it pins (`npm-smoldot-v3.3.2`). The 2026-07-11 figure stands as the record of that date and is not edited; this is the re-verification, and where the two differ the lockfile wins. Details and the FE-P1…P12 prototype gates live in [10](10-frontend-architecture.md) and [12](12-release-and-operations.md).

...
- POL MUST remain undisturbed through the decision window (a decision-grade condition, doc 05); its live NAV commitment ends at settlement, independently of the retained-book archive slot. **The custody returns with the commitment (normative; added 2026-07-29, milestone E1):** the book account's surviving complete sets — worth exactly one branch-USDC (resp. one USDC) per set at any settlement — are redeemed and returned to `POL` for decision and gate books, `POL_BASELINE` for the Baseline book, by the §2 **Sweep** stage, per [08](./08-treasury-and-economics.md) §8 step 5(b). Realized cost = divergence loss in the live branch = the explicit information subsidy — an identity that holds **only if** the custody returns. Releasing the NAV obligation while leaving the seed escrowed makes the realized cost the entire seed, turns the largest revolving balance in the system into a recurring expense, and overstates NAV in the over-permissive direction ([08](./08-treasury-and-economics.md) §8 step 5, §10.5).

...
| **Sweep** *(added 2026-07-29, milestone E1)* | any block on which the book is closed **and** its owning vault is terminal (`ScalarSettled`, `Voided`, or Baseline `Settled`); strictly before Reap. The conjunction is reachable on every terminal path and cannot deadlock: the terminal latch of "Reap interleavings" below is written atomically with the ledger transition and freezes the book, so a proposal voided *before* decision close is sweepable on the same rule as one that closed at d18. **Additionally requires the owning ledger domain's payout-safety freeze to be inactive** ([06](./06-governance-and-guardians.md) §6.3; [16](./16-hosted-question-service.md) §10; errors `Frozen`): Protocol books consult the primary `PB-LEDGER-FREEZE`; external books consult only the service instance's own I-4 freeze. The crank redeems the fee account's claims through an internal ledger path, so bypassing its own domain's freeze would pay the protocol's claim out of a possibly-short sovereign while that domain's claimants are refused; consulting the other domain would instead violate I-37 and strand independent capital. An already-swept book still answers `Ok` — idempotence is checked first, so the marker's meaning never depends on either freeze state | keeper `sweep_revenue(market)`, Signed and permissionless | Realize the book's protocol revenue and return its subsidy custody, **atomically**: the fee account's realizable claims redeem to USDC and remit **100% to the treasury `MAIN` account** (§6.1; [08](./08-treasury-and-economics.md) §1.1), and the book account's **every realizable** book-owned position redeems to USDC and returns to `POL` (decision and gate books), `POL_BASELINE` (Baseline), or the immutable exact funder (external, §3) — pair redemption wherever a pair exists, then the unmatched legs individually, leaving for Reap **only** provably zero-payout positions — [08](./08-treasury-and-economics.md) §8 step 5(b). Scoping this to complete sets alone would strand the unmatched residual leg, which at an interior `s` is worth `floor(a·s)`/`floor(a·(1−s))` and is precisely the inventory that makes realized cost the divergence loss rather than the whole seed. Idempotent (the swept marker makes a repeat a no-op), **fail-soft** (it can never fail a settlement — G-1), and O(1)-bounded: both accounts hold at most the owning vault's fixed 14-instrument universe (2 for Baseline). Emits domain-correct `RevenueSwept` / `ExternalRevenueSwept` (§11) |
| **Reap** | settlement + `ledger.archive_delay`, **and the book's swept marker set** | keeper `reap(market)`; ledger `sweep_dust*` is independent | Atomically discard the book/fee accounts' fixed protocol-position universe (≤28 proposal cells or ≤4 Baseline cells), unregister them and remove the `Markets` row; claimant positions/vault collateral remain untouched and history survives via events (§11) and `RecentCohortSummaries` (doc 02). What reap discards is by construction the **worthless residue** the Sweep stage left — losing-branch and unrealized-branch legs that pay 0 at any settlement. **A book that has not been swept is not reapable** and reap MUST refuse it |
...
**Reap interleavings (normative).** The ledger's vault reap and the market's book reap are both permissionless and MUST be safe under any call ordering. The ledger's terminal markers are themselves swept state and MUST NOT serve as the market's POL-obligation predicate. Instead, every ledger transition that records a terminal block (`void`, `settle_scalar`, `settle_baseline` — [03](./03-conditional-ledger.md) §5.4) MUST, in the same atomic storage layer, latch that block into the owning market for each of the vault's books, release the domain's active slot and any protocol-domain POL commitment, and delete the terminal book's decision-window/checkpoint auxiliaries. The latch is durable until the book is reaped. A seeded protocol book's POL obligation is live **iff** no latch is recorded; market reap MUST require the latch plus `ledger.archive_delay` elapsed since it. Book/fee addresses come from the runtime's canonical domain-separated `AccountId32` namespace, are protocol-classified permanently (including before creation and after reap), and creation rejects any other pair before mutation; the refcount index records ownership rather than granting the exemption. Signed transfers into protocol accounts are forbidden and market reap MUST atomically discard only those two accounts' balances across the fixed owning-vault instrument universe before unregistering them (14×2 proposal-shaped cells, including external, or 2×2 Baseline cells; [03](./03-conditional-ledger.md) §5.4). It neither waits for nor deletes claimant positions or vault collateral. Thus market-first and ledger-first both succeed safely, and unrelated claimant traffic cannot enlarge the fixed work. The latch MUST be idempotent, and marker, latch, active-slot release and any POL release MUST commit or roll back **together** (status-quo default, G-1).

**What reap discards, and what it must not (normative; amended 2026-07-29, milestone E1).** That discard applies to the **residual, worthless** inventory the Sweep stage left behind — the losing-branch and unrealized-branch legs that pay 0 at the recorded settlement — and not to the book's inventory as such. `sweep_revenue` realizes the valuable part first: the fee account's claims and the book account's every realizable surviving position are each worth real USDC at any settlement (§6.1, §6.3), and the Sweep stage converts them and remits them to `MAIN` plus the funding domain (`POL`/`POL_BASELINE` or the external exact funder) respectively. Discarding **un-swept** inventory is therefore not a cleanup but a silent transfer: it converts funder capital into ledger residue, which [03](./03-conditional-ledger.md) §5.4's `sweep_dust*` then routes to `INSURANCE` — an account [08](./08-treasury-and-economics.md) §1.2 places outside NAV, whose only exit is a governed `sweep_insurance` that itself consumes a proposal slot. [08](./08-treasury-and-economics.md) §10.5 records what that cost while it was the implemented behavior, and is the reason this rule is stated rather than assumed.

**Sweep/reap ordering (normative).** `sweep_revenue` and `reap` are separate permissionless cranks and MUST be safe under any call ordering and any number of repetitions. `sweep_revenue` is **idempotent**: the swept marker is written in the same atomic storage layer as the two remittances, so a second call is a successful no-op rather than a second payment, and a partially-applied sweep is unreachable. `reap` requires that marker **in addition to** the terminal latch and `ledger.archive_delay`, and MUST refuse a book that lacks it rather than proceeding; reap-before-sweep is the one ordering that must not be reachable, because unlike every other interleaving here it is irreversible. The marker is durable until reap and is removed with the `Markets` row. A book whose sweep has not yet run is not a solvency problem — the value is still held as fully collateralized protocol positions — it is an NAV-recognition delay of exactly the kind [08](./08-treasury-and-economics.md) §8 step 5 bounds. **An active owning-domain payout-safety freeze therefore delays reap transitively** (primary `PB-LEDGER-FREEZE` for Protocol; service-instance I-4 freeze for External): that domain's freeze refuses `sweep_revenue`, the missing marker refuses `reap`, and both resume when that same domain becomes payout-safe. The two instances never gate one another (I-37). This is inert rather than harmful in both directions — reap only ever discards worthless residue, while settlement and the VOID terminal path remain live.
...
**Capacity and pair lifecycle are separate by construction.** A question is created atomically as
exactly `External{question, client, Accept}` plus `External{question, client, Reject}` against one
service vault; there is no singular external-book constructor and the protocol pair-seed shortcut is
not admissible for this domain. Each book posts its own `b·ln 2` headroom, matching [16](./16-hosted-question-service.md)
§8.2's `2·b·ln 2` cash expression. Pair seeding is atomic: it performs two funder splits, moves
`2·b·ln 2` of each branch out of the funder's signable account, scalar-splits one headroom in each
matching book, and leaves the other headroom as raw same-branch inventory in that book. The client
therefore retains no Accept+Reject pair it could merge before terminal settlement; the winning raw
branch returns through the same exact-funder capability as the realizable scalar inventory, while a
VOID returns both raw half-claims. No book is marked seeded unless every move commits.
External books increment only
`ActiveExternalMarketCount`, whose live limit is `2·svc.max_live` and whose hard ceiling is 128; they
never increment `ActiveMarketCount`, enter `LivePolCommitments`, or consume `MaxLiveMarkets = 196`.
The live limit is an **admission** ceiling: lowering `svc.max_live` refuses new pairs but does not
...
`crank_observe` and `sweep_revenue` select the owning instance through `LedgerRoute::for_book`:
Protocol books consult the primary market/ledger I-4 freeze and External books consult the service
ledger's independent I-4 freeze. An unavailable service-status binding is frozen (fail closed).
Neither instance's latch may stop or admit the other's funds movement (I-37), and the service pause
of [16](./16-hosted-question-service.md) §10 is not this latch. `PB-DEPEG`'s
`CreationFrozenUntil` remains global because it guards new shared-USDC market creation, not an
instance's payout safety; both domains' create/seed paths obey it.
...
**Fees.** `mkt.fee = 30 bps` (*normative value: §13*), non-refundable on all paths. On `buy` the fee is collected as a **complete branch-USDC pair** (both legs to `fees_accrued`) — worth exactly `fee` USDC at any settlement, so fee income is unconditional. Under **branch annulment** the buyer's loss is exactly the fee; under **protocol VOID** it is not — total buyer delta also carries the difference between the package's D-1 neutral value and `cost`, so PT-2 MUST NOT collapse VOID delta to `−fees` alone (§6.2; SQ-171). On `sell` the fee is withheld single-sided in target-branch bUSDC; its realized value follows the branch (0 if voided — a protocol-side income haircut, never a trader-side charge beyond 30 bps). **Realized fee value routes 100% to the treasury `MAIN` account** (amended 2026-07-29, milestone E1; the former 50% `INSURANCE` / 50% POL-offset split is superseded — [08](./08-treasury-and-economics.md) §1.1 gives the two dispositive reasons, and the POL-offset leg is subsumed rather than lost because `POL` is funded from `MAIN`). Realization is the §2 **Sweep** stage: the permissionless `sweep_revenue(market)` crank redeems the fee account's claims to USDC once the owning vault is terminal and remits them. Collection is **automatic and permissionless** and MUST NOT require a proposal slot. Economics and sizing: [08](./08-treasury-and-economics.md) §1.1 and §10.

The market fee is the *first* of the protocol's two revenue instruments and the only one this document owns. Settlement payouts additionally carry the ledger-side redemption fee `ledger.redeem_fee` ([03](./03-conditional-ledger.md) §5.3a; economics [08](./08-treasury-and-economics.md) §10.2/§10.6), which is charged on the scalar, gate and Baseline redemption calls and is **exempt** on the par leg `redeem`, on `redeem_void` and on every `merge*` — so §6.2's annulment and VOID statements, and the G-3 promise they rest on, are unchanged by it.

**Baseline books** are unconditional: the wrapper degenerates — `cost + fee` USDC pays in directly, `split_scalar` against the epoch's Baseline vault mints unconditional LONG+SHORT sets, and there is no mirror credit (§8.2). **The buy-side fee is segregated into the book's fee account at trade time, as a complete LONG+SHORT Baseline set of size `fee`** (normative; added 2026-07-29, milestone E4, SQ-519). Two properties make this the only admissible mechanism. *Segregation, not accounting:* the fee must leave the book's LMSR inventory when it is charged, because by settlement that inventory may no longer cover the accrued fee — a book whose bought leg was drawn down by traders would have nothing left to remit, so a sweep-time computation from a running total is not realizable. This is exactly why decision and gate books already route to a fee account at trade time, and the Baseline wrapper's degeneracy is not a reason to differ. *Ordering:* the segregation happens **before** the buyer's leg is paid out, so a book that cannot fund `amount` after setting the fee aside genuinely cannot fund the trade, and the trade fails (G-1) rather than silently financing itself from fee revenue. At Sweep the fee account's complete set redeems to `MAIN` — a complete Baseline set pays exactly 1 USDC at any `s_e` (§8.2), so it realizes at par with no settlement dependency, and the fee account is a protocol account, which [03](./03-conditional-ledger.md) §5.3a exempts from the redemption fee (the deduction applies to a **non-protocol** claimant), so the protocol's own fee realization is not charged its own fee.

*Why this was wrong before E4.* The implementation split the whole `cost + fee` into sets held by the **book**, indistinguishable from seed inventory, so `withdraw_fees` returned zero for Baseline and `withdraw_book` returned the fee to `POL_BASELINE` with the subsidy. That is not a solvency defect — the value stays fully collateralized inside NAV — but it defeats the instrument for an entire book class, since `POL` is committed liquidity and only `MAIN` is spendable against the recurring cost base ([08](./08-treasury-and-economics.md) §1.1/§10). It also made `pol_returned > pol_seeded`, which falsifies §10's "realized cost = divergence loss" identity in the opposite direction. [08](./08-treasury-and-economics.md) §10.2's `ρ` already counted Baseline books at realization **1.0 on both sides**, so the arithmetic assumed this routing before the code delivered it. The sell side degenerates too, and in a way that matters for custody: with no mirror leg to merge against, the **book** (not the seller) funds the payout set, so it merges `net + fee` and re-splits `net`, **retaining the fee as plain USDC** rather than as branch-USDC. A Baseline book is therefore the one per-market account that holds real collateral, which is why [03](./03-conditional-ledger.md) §7 R-4 endows it with `min_balance` at Seed: below that floor the re-split fails on the ledger's `Preservation::Preserve` custody rule and an ordinary small sell is rejected. **That retained plain-USDC fee balance is remitted to `MAIN` by the same `sweep_revenue` crank** (§2; added 2026-07-29, milestone E1) — it needs no redemption leg, being already USDC — which closes the **fee component** of the residue [03](./03-conditional-ledger.md) §7 R-4 records as currently unrecoverable after Baseline-book reap. It does **not** close the `min_balance` component and MUST NOT be read as doing so: the endowment is the account's existential floor, every protocol path out of the account preserves it, and the sweep therefore remits only the balance above `min_balance` and leaves the floor exactly where R-4 puts it.
...
**Every Baseline book reaches a terminal latch (normative; SQ-320).** The measured e+3 settlement is not the only way the epoch's Baseline vault terminates, and the book's reapability depends on it terminating: §2's reap ordering makes a seeded book's POL obligation live until `settle_baseline` writes the terminal-block latch, so a Baseline vault that never settles leaves its book permanently un-reapable and its POL permanently committed — and, since milestone E1, permanently *un-returned*: the latch is also the precondition of the §2 Sweep stage, so an unsettled Baseline strands the `POL_BASELINE` custody as well as the NAV obligation. The two neutral paths of [05-welfare-and-decision-engine.md](./05-welfare-and-decision-engine.md) §7(5)–(6) — the epoch-VOID cohort void, and the permissionless orphan-epoch finalization for an epoch whose cohort never formed — settle the vault at `s = 0.5` and write the same latch through the same `settle_baseline` call, so no market-side rule changes and no third neutral path exists. Ledger-side ownership of the neutral settlement is [03-conditional-ledger.md](./03-conditional-ledger.md) §5.2.
...
- POL MUST remain undisturbed through the decision window (a decision-grade condition, doc 05); its live NAV commitment ends at settlement, independently of the retained-book archive slot. **The custody returns with the commitment (normative; added 2026-07-29, milestone E1):** the book account's every realizable surviving position — worth exactly one branch-USDC (resp. one USDC) per set at any settlement — are redeemed and returned to `POL` for decision and gate books, `POL_BASELINE` for the Baseline book, by the §2 **Sweep** stage, per [08](./08-treasury-and-economics.md) §8 step 5(b). Realized cost = divergence loss in the live branch = the explicit information subsidy — an identity that holds **only if** the custody returns. Releasing the NAV obligation while leaving the seed escrowed makes the realized cost the entire seed, turns the largest revolving balance in the system into a recurring expense, and overstates NAV in the over-permissive direction ([08](./08-treasury-and-economics.md) §8 step 5, §10.5).
...
- Per-class budget arithmetic, the 0.75%-NAV epoch cap, minimum-viable NAV, and Ask-scaled `b` (D-4) are owned by doc 08; fee routing — **100% of realized fee value to the treasury `MAIN` account** since milestone E1, superseding the 50% `INSURANCE` / 50% POL-offset split ([08](./08-treasury-and-economics.md) §1.1; realized by §2's Sweep stage, §6.1) — likewise.
...
| `RevenueSwept` | `{ market: MarketId, fee_to_main: Balance, pol_returned: Balance }` | One per **successful** `sweep_revenue` (§2; added 2026-07-29, milestone E1). **Both amounts are real USDC** — never branch-USDC, complete sets or position units: `fee_to_main` is what the fee account realized and remitted to the treasury `MAIN` account, `pol_returned` what the book account's every realizable surviving position returned to `POL`/`POL_BASELINE`. Either MAY be `0` (a wholly annulled sell-side fee; a book whose inventory the §6.3 drain consumed). Because the crank is idempotent, a repeat run is a silent no-op and exactly one `RevenueSwept` exists per market |
| `ExternalRevenueSwept` | `{ market: MarketId, fee_to_main: Balance, subsidy_returned: Balance }` | External-domain counterpart (§3): the fee leg has identical `MAIN` ownership, while `subsidy_returned` went only to the book's immutable exact funder and never to POL. Either MAY be `0`; idempotence permits exactly one per external market |

`Traded` and `Observed` are the frontend's entire price-history pillar (event-derived, chain-served — D-2/D-6); they are load-bearing and MUST NOT be gated behind an indexer. Emission is unconditional on success paths; `close`/`reap` and settlement events are enumerated in doc 02, where all four shapes above are frozen.
...
| **E1** — protocol revenue routing (2026-07-29) | §6.1: realized fee value routes **100% to the treasury `MAIN` account**, superseding the 50% `INSURANCE` / 50% POL-offset split, with collection automatic and permissionless; §2: the new **Sweep** stage `sweep_revenue(market)` — Signed, permissionless, idempotent, fail-soft, O(1) over the owning vault's fixed instrument universe — realizes that fee value and returns the book's surviving POL inventory to `POL`/`POL_BASELINE`, and is a precondition of reap, which now discards only worthless residue; §10: the POL custody returns with the NAV commitment, without which "realized cost = divergence loss" is false; §6.1: the Baseline book's retained plain-USDC fee is remitted by the same crank, closing the fee component of the [03](./03-conditional-ledger.md) §7 R-4 residue; §11: `RevenueSwept`. Economics and the redemption-fee counterpart: [08](./08-treasury-and-economics.md) §1.1/§8 step 5/§10 and [03](./03-conditional-ledger.md) §5.3a |

...
| **Collators** | 5 invulnerables → 8–12 bonded permissionless (Phase 4+); geographic/organizational diversity feeds the Security pillar; compensation 500 USDC/collator/epoch *(normative value: [08](08-treasury-and-economics.md) §2.4; re-anchored from 2,000 on 2026-07-31, SQ-535 — the counts in this row are what this document owns, the rate is not)*. **Pre-arming gate (normative; 2026-07-31, SQ-535):** the rate is anchored to what operators already running Polkadot infrastructure accept, i.e. a *marginal* cost, so before the **bonded permissionless** set here is armed it MUST be re-verified against operator quotes obtained for this chain and amended if they exceed it. The invulnerable launch set is the only period in which the market rate does not bind, and the transition above is scheduled at Phase 4+, so this is owed early | Collator program coordinator | `ops.collators` |

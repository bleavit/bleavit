...
Pinned Node version via container digest; **`pnpm install --frozen-lockfile`** from the committed lockfile, with the `packageManager` field pinned and `node-linker` fixed to the isolated (symlinked) layout; `SOURCE_DATE_EPOCH` fixed; Vite configured for content-hash-only filenames and stable chunking; a post-build normalizer strips nondeterminism (plain tree, no archive metadata).

> **Why pnpm, and the constraint it carries.** The package manager is not a preference here: [10 §10.2](10-frontend-architecture.md) requires a forbidden import to *fail module resolution*, and under a hoisted `node_modules` an undeclared import resolves anyway — only `tsc -b` objects, which demotes the primary firewall gate to the secondary one. Only an isolated layout makes that section's claim literally true.
>
> The reproducibility risk this raises — pnpm's content-addressed store leaking absolute paths into sourcemaps — was measured before adopting it, not assumed away. Two builds at different absolute path depths with **separate stores** produced a byte-identical tree and zero absolute paths in output: pnpm's virtual store lives inside the project at `node_modules/.pnpm`, so emitted sources are relative. **But those relative paths encode the store layout** (`../../node_modules/.pnpm/<pkg>@<version>/node_modules/<pkg>/…`), which is exactly why `node-linker` is part of the pinned recipe rather than a default: a hoisted install would build the same bytes of application code and a different sourcemap, and the two-environment gate below would fail for a reason with nothing to do with the source.
...
- **Primary: withdrawn. Native ANT n-of-m does not exist** *(FE-P7's first half resolved 2026-08-07; SQ-940)*. An ANT authorizes an action by **membership**, not by threshold: the AO implementation's `assertHasPermission(from)` returns as soon as `from` matches any one entry of `Controllers`, or the owner, or the process itself, and `assertHasRecordPermission` adds the per-record owner and a balance holder on the same any-one basis. A controller list is therefore **1-of-m**, and adding controllers *widens* custody rather than splitting it — the opposite of what D-16 asks for. The current Solana implementation restates the same shape as a *"controller ACL"*. No wording of `@ar.io/sdk` changes this, because the check runs in the program and not in the client.
- **Fallback, now the only path:** a **threshold ceremony producing a single controller principal** whose signatures require 3-of-5 of the share-holders. This was specified as FROST-ed25519 **[VERIFY the threshold scheme against the signature scheme the current ANT program actually verifies — SQ-940]**, and that verification is now a different question than when it was written, because the platform changed (below). The general form is unchanged and is what D-16 requires: **the threshold lives outside the ANT**, in whatever principal the ANT recognizes as its owner or sole controller.
- **Prohibited:** single-key custody of the production ANT, under any circumstance, including "temporarily" during bootstrap. **If the threshold ceremony does not materialize, launch blocks on this line** — the production name does not go live (phase-gate wiring §6.5). The primary option is gone, so this line no longer has two ways to be satisfied.

**The platform this section describes has moved, and that is a larger finding than the gate asked for** *(SQ-940, 2026-08-07)*. `ar-io/ar-io-ant-process` — the AO ANT implementation — was archived on 2026-06-09 and states: *"The AR.IO smart contracts have migrated from AO to Solana. All smart contract code now lives at ar-io/ar-io-solana-contracts."* The ar.io documentation now reads *"the registry is managed on Solana by the `ario-arns` program"*, and `ario-ant` is published as *"Arweave Name Token (ANT) as a Metaplex Core NFT with Attributes plugin, undername records, controller ACL"*. Data storage is unaffected — bundles still go to Arweave through Turbo, and §1.2's two-pass flow is untouched. **Name control is what moved.** Every "controller address", "controller share", `setRecord` call and threshold-scheme compatibility claim in §1.4, §2.2 and this section is now a statement about a Solana program rather than an AO process, and each must be re-derived before the §6.5 Phase-3 ceremony rather than carried over. FE-P7's four remaining halves (two-pass manifest flow, undername immutability, resolver endpoints, manifest fallback) must be re-asked against the current platform before a dry run is designed.
...
**The external-tool handoff adds nothing to this list, and a release MUST NOT add an external-tool vendor host to it (D-21).** The handoff of [10 §13](10-frontend-architecture.md) is transported by the user agent and the operating system — files, the async Clipboard API, the platform share sheet, inbound deep links into the app's own origin — none of which is a fetch, so none is governed by `connect-src`. Outbound links to named tool vendors are top-level navigations rather than fetches and likewise add no entry; the vendor list ships inside the signed release and is never fetched (INV-FE-13). The handoff packages contain no network primitive at all, and the build asserts both halves mechanically: a source gate rejecting `fetch`, `XMLHttpRequest`, `WebSocket`, `EventSource`, `sendBeacon` and dynamic URL `import()` in those packages, and a diff of the emitted allowlist against the incumbent release's that fails on any addition. This rule is stated here, at the allowlist's home, because a vendor host is exactly the kind of entry that arrives one release at a time.
...
| **Collators** | 5 invulnerables → 8–12 bonded permissionless (Phase 4+); geographic/organizational diversity feeds the Security pillar; compensation 500 USDC/collator/epoch *(normative value: [08](08-treasury-and-economics.md) §2.4; re-anchored from 2,000 on 2026-07-31, SQ-536 — the counts in this row are what this document owns, the rate is not)*. **Pre-launch gate (normative; 2026-07-31, SQ-536, corrected same day):** the rate is anchored to what operators *already running Polkadot infrastructure* accept, i.e. a **marginal** cost, so it MUST be re-verified against operator quotes obtained for this chain — and amended if they exceed it — **before production launch and before each enlargement of the set, the invulnerable launch set included**. Invulnerability governs *selection*, not willingness to author, so it is not an exemption; and the launch operators are the most likely to be standing up infrastructure for this chain specifically, which is the case the anchor does not cover. This is a **mechanically enforced** release blocker — the `collator.rate_unverified` row of `tools/release/surface-manifest.json`'s `release_blockers`, which fails a tagged release closed — cleared by recording the quotes in PLAN.md's Verification log (V-61) | Collator program coordinator | `ops.collators` |
...
| **Hosted question service** | Client delivery, occupancy against the §8.5 resource partition, and the [16](16-hosted-question-service.md) §8.4 cannibalization falsifier; funded on the keeper line because the service's only recurring cost is the crank load of its own books | Service operator | `ops.keepers` |
...
| Collateralization | per-instance Σ escrow vs sovereign balance drift | any drift ≠ 0 | RB-LEDGER (page immediately) |
| Treasury | NAV, meter utilization, stream schedule | meter > 80% | RB-TREASURY |
| XCM | send/fail/timeout counters, trapped assets, probe funding readiness | any trap or probe runway unready/unobservable | RB-XCM |
...
| Client report push | pushes attempted, pushes failed, consecutive failures per client | any client with ≥ 3 consecutive push failures | RB-SERVICE |
| Hosted service occupancy | live external questions, `svc.max_live` headroom, external vs Bleavit contest capital, `NotDecisionGrade` rejections/epoch | live ≥ 0.9 · `svc.max_live`, **or** `NotDecisionGrade` rejections rising with external occupancy | RB-SERVICE |
| External block-weight quota | external ref-time and proof-size used vs the reserved external quota, per block | > 80 % of either quota | RB-SERVICE |
...
**Hosted-service row semantics (normative; added 2026-08-01, D-20; N9 metrics bound 2026-08-02).** Three readings that the terse cells above do not carry, and that get them wrong if guessed. **Client report push is not a health signal about this chain.** Its three per-client series are monotone attempts, monotone total failures and a consecutive-failure gauge reset only by a successful push; local clients, for which push is inapplicable, emit none. The counter is deliberately **outside** welfare: I-36 forbids an externally-triggered send from feeding `X`, precisely because a client that never opens its return channel could otherwise move a Bleavit decision input at zero cost. The alert exists so operators can tell an integrating client that its pushes are failing — the client's *authoritative* delivery is the pull surface, verified by storage proof against a finalized header. **The `NotDecisionGrade` clause is a falsifier, not a threshold.** [16](16-hosted-question-service.md) §8.4 states the standing instruction plainly: if Bleavit proposals start failing their depth floor as external occupancy rises, the values layer MUST reduce `svc.max_live`. The alert is what makes that instruction observable; it is the only monitoring row in this document whose response is a **parameter cut** rather than an operational action. **The external weight quota is a partition, not a budget.** It fires on the reserved *external* allocation, never on total block utilization — [05](05-welfare-and-decision-engine.md) §4.3's `H` is computed against the reserved **primary** capacity, so an exhausted external quota is the system working as designed and a *breached* one is a partition defect.
...
- **Reserve-probe runway (XCM-domain extension).** The XCM domain MUST additionally expose the local `ops.reserve_probe` balance as full-envelope runway and independently observe the Asset Hub sovereign account's usable USDC and usable DOT balances. It alerts through RB-XCM before the local line falls below `res.fail_threshold + res.recover_threshold` full debits, before remote USDC falls below `res.probe_amount`, or before remote DOT falls below that same number of full `ops.probe_fee_dot` envelopes **plus the operations refill margin**. The margin is a mandatory positive operator setting in DOT plancks, supplied to the exporter rather than invented as a protocol parameter; the maximum age of an unchanged finalized Asset Hub head is likewise a mandatory positive finite operator setting in seconds, and the operator MUST pin the canonical Asset Hub genesis hash. The endpoint must also report the frozen Asset Hub para id 1000. Freshness advances only with a strictly increasing finalized height; a regression or a different hash at the same height is invalid. The parameterized thresholds are evaluated from live Params and the remote balances are never inferred from the local accounting line. Asset-wide USDC status must be `Live` and the sovereign account status `Liquid`; frozen/blocked/destroying state is not usable inventory. Absent RPC responses, malformed values/metadata, stale heads, identity mismatch, or disconnects clear every actual/required/readiness series and assert an explicit unhealthy collection gauge; they never become a healthy zero. A successful finalized query proving that the canonical account row does not exist is an observed zero balance: it asserts collection health but fails readiness. This is one XCM-domain funding-readiness signal, not a twenty-second monitoring domain; its implementation must extend the existing XCM rule and inventory atomically so the §6.3 coverage gate remains exact.
...
|---|---|
| F-5 | §4: ArNS permabuy eliminates lease-lapse takeover; ANT control is 3-of-5, held by a threshold ceremony **because the capability [VERIFY] resolved negative** — no native ANT n-of-m exists, so that option is withdrawn (§4.2, SQ-940); single-key custody prohibited; launch blocks on this line. |

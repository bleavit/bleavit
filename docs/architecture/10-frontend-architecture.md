...
- **The conservation replay drops.** It requires non-negativity at every step from a zero start, and a page that opens mid-history legitimately consumes positions minted before it. This is the impossibility above, and it is the screen that carries it.

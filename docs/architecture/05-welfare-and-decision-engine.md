...
**The bound is 20 epochs, and the storage bound is 20 × `epoch.horizon_k` records (normative; 2026-07-27).** The retained window above is stated — correctly — in *epochs*, and so is [13](./13-parameters.md) §4's `Snapshots` row. But `Snapshots` is keyed `(epoch, spec_version)` and capacity is enforced against the **record** count, so the epoch bound has to carry §3.3's version multiplicity to mean the same thing: `MAX_SNAPSHOTS = 20 × 2 = 40`. It did not, and the two rules then disagreed by exactly the factor §3.3 says every derived `× 2` rests on. The consequence was not a capacity nuisance but a permanent wedge: across a MetricSpec activation boundary the epoch legitimately needs both the in-flight cohort's frozen-version record (I-16) and the active-version one, which is 21 lawful records against 20 slots — so a signed caller could spend the epoch's single spare slot on the cohort record before the keeper wrote the deadline-advancing one, after which `SnapshotDeadline` never advances, §4.8's dead-man latches, and the frozen epoch clock freezes the very prune cutoff that would have released the slot. §4.8 (SQ-254) already names that class: a cause "no crank can clear … (no origin can clear the flag)". The retained **epoch** window is unchanged, so the epoch-keyed bounds derived from it — gate-breach flags, and [13](./13-parameters.md) §4's 21-epoch shared prefix index — keep their values; only the record bound moves. `MaxSnapshots` is a metadata constant the frontend reads, so the change is carried as [02](./02-integration-contract.md) contract **v16**.

...
| `0x0C` | Community distribution | `futarchy_treasury.create_community_schedule(beneficiary, amount)` ([08](./08-treasury-and-economics.md) §2.1) | — (singleton) |
| `0x0D` | Trading-reward funding | `futarchy_treasury.fund_trading_rewards(amount)` ([08](./08-treasury-and-economics.md) §2.6) | — (singleton) |
...
**Why the insurance reserve is a singleton family (normative; SQ-207).** `sweep_insurance` names no beneficiary and no budget line — [08](./08-treasury-and-economics.md) §1.2 fixes its destination as `MAIN` and §1.4 makes it an inbound transfer rather than an outflow — so neither the `0x07` beneficiary nor the `0x09` budget-line discriminator applies. The contended resource is the single INSURANCE custody balance, and two concurrent sweeps genuinely contend on it, so one lock over the account is the correct grain; as with `0x0A`, serialising costs only liveness while under-locking would be a safety cost, and serialising is the G-1 direction. **Ask derivation:** the sweep moves USDC *into* NAV and creates no treasury outflow, so its derived Treasury ask is exactly **zero** — the second of the two admissible zero-outflow Treasury leaves named above. **Class safety:** the leaf is payload-admissible only for `ProposalClass::Treasury` carrying the dedicated `InsuranceSweep` capability, matching the [06](./06-governance-and-guardians.md) §3.2 row; no other class may inherit it.

**Why community distribution is a singleton family (normative; B20/SQ-107).** `create_community_schedule` mutates the single remaining-allocation and lifetime-count pool even when beneficiaries differ. A beneficiary-derived key would therefore allow two PARAM proposals to pass T5 concurrently and make the second fail only after adoption when the finite pool is consumed. The `0x0C` key serialises every community schedule; this costs only liveness and is the G-1 direction for a bounded solvency-critical allocation. The leaf remains PARAM-origin-only under [08](./08-treasury-and-economics.md) §2.1 and carries no frozen 02 ingest surface.

**Why trading-reward funding is a singleton family (normative; added 2026-08-10).** The `0x0C` argument applies here unchanged, and it is restated rather than cross-referenced because the two calls contend on different pools. `fund_trading_rewards` mutates the single remaining-allocation and lifetime-count pool of the genesis `incentiv` allocation, whatever amount it names. A key derived from the amount, the epoch or the recipient pallet would therefore let two PARAM proposals pass T5 concurrently and make the second fail only after adoption, once the finite pool is consumed. The `0x0D` key serialises every authorization. Serialising costs only liveness, and under-locking a bounded solvency-critical allocation is a safety cost, so serialising is the G-1 direction. The leaf is PARAM-origin-only under [08](./08-treasury-and-economics.md) §2.6 and carries no frozen 02 ingest surface. **Ask derivation:** the call moves VIT between two protocol-owned accounts and creates no treasury outflow of NAV-bearing USDC, but it is **not** a third zero-ask Treasury leaf, because it is a PARAM leaf and never dispatches under a Treasury origin at all.
...
**Lock persistence to dispatch.** Locks acquired at T5 are released at the point the proposal becomes **dispatch-unreachable** — for the ordinary path that is its terminal transition, and every terminal transition of a queued proposal cancels its queue entry atomically (§2.1, [doc 09](./09-execution-upgrades-and-rollout.md) §1.1) — so [doc 09](./09-execution-upgrades-and-rollout.md) §1.2(8)'s "locks still held" is structural on the epoch side; the guard's step-8 meter locks and step-11 call-domain re-derivation (I-11) re-check the mechanically derived surfaces at dispatch time.

**T10 rejection releases immediately (normative; SQ-318 resolution, 2026-07-24).** Composing the sentence above with §2.1's *Terminal states* paragraph once implied that a T10-rejected proposal with a healthy vault holds its resource domain for the whole ~3-epoch measurement window, since that state is classified **transient** (terminal only at `Settled`). It does not, and must not: this rule's subject is persistence *to dispatch*, and a `Rejected` proposal can never dispatch. Both rerun entries exclude it by construction — the guardian `force_rerun` admits only `Trading`/`Extended`/`Queued`, and the keeper `schedule_rerun` only `Suspended` — so no path carries a rejected proposal back to a dispatchable state. Holding the lock past that point protects nothing and denies the domain to every other proposal that would touch it, which §1.4's own collision paragraph already classifies as the kind of pure **liveness cost** the design avoids where it buys no safety. Ruled: **T10 releases the proposal's resource locks in both arms — with markets and without.** Measurement, cohort membership and vault resolution are unaffected; they concern welfare scoring, not the resource domain.
...
- **Full refund + cancel** — everything else, including: a payload with **no classifiable domain and an empty resource declaration** (a verifiable no-op / domain mismatch, which 06 §4 does not name as confiscable), an undecodable payload, a call the classifier cannot re-derive while its footprint is otherwise admissible, **a call the classifier *can* re-derive but whose projected domain the execution guard's `enqueue` refuses, while the declared footprint matches and the class capability check passes** (SQ-362 resolution, 2026-07-24 — the payload's only fault is a classifier projection artifact, typically a nested `utility.batch_all` that §1.4 explicitly blesses; the section's governing principle is that confiscation requires a verified culpable act, so the refund arm is the default and the two slash arms remain the enumerated exceptions), no bond floor defined for the derived class, a bond that fell below a floor raised after submission, and a TREASURY ask that does not match its declared in-cap prize.
...
**Measurement-time cohort VOID has no proposal transition (normative; SQ-162).** Its absence from this table is deliberate, not a gap. When a cohort VOIDs during measurement ([doc 07](./07-oracle-and-disputes.md) §10, §7 below), the **cohort** moves to `CohortInfo.status = Void` and the affected **vaults** move `Resolved → Voided` ([doc 03](./03-conditional-ledger.md) §2.3) — the proposals themselves do not move, and **their recorded decisions stand** (07 §10: "decisions already made stand"). A `Measuring` proposal has already been decided and executed or rejected; overwriting that decision would falsify the archive, which is the only durable record of what the market concluded. T20 is **not** the transition for these proposals: T20 is scoped to *pre-Executed* states and applies, in the same epoch-VOID transaction, to that epoch's other affected proposals. Normatively: **on measurement-time cohort VOID an already-decided proposal MUST retain its state and its recorded `DecisionOutcome`, and MUST NOT emit a per-proposal rejection event**.

**Decided-but-pre-Executed proposals (normative; SQ-319 resolution, 2026-07-24).** The sentence above once read "only that epoch's proposals which have **not yet been decided**" take T20, which contradicted the T20 row's own scope: T9 records `Some(Adopt)` on entry to `Queued`, so `Queued`, `Suspended`, `Rerun` and `FailedExecuted` are simultaneously *pre-Executed* and *decided*. The contradiction dissolves once the two fields are read as the different facts they are: **`state` is the process outcome, `DecisionOutcome` is what the market concluded.** Ruled: T20's scope is unchanged — every affected non-member is halted, takes `state = Rejected(ProcessHold)`, emits `ProposalForceRejected` and can never execute — but it **MUST NOT overwrite a `DecisionOutcome` already recorded**; it writes `Reject(ProcessHold)` only where none exists. A cohort VOID invalidates *measurement inputs*; it is not evidence that the decision went the other way, and recording a rejection the market never produced falsifies the archive — the same truth principle batch D ratified for the `Measuring` case (SQ-314).
...
`CohortInfo { epoch, proposals: BoundedVec<ProposalId, 5>, status: Measuring{until} | AwaitingOracle | Settling{cursor} | Settled | Void }`. At most 4 cohorts non-terminal simultaneously (2 measuring + 1 awaiting oracle/challenge + 1 settling) — I-21. Measurement horizon **k = 2** (default): cohort e measures over epochs e+1 and e+2; Snapshot(e+2) finalizes and survives its **72 h** challenge window ([doc 07](./07-oracle-and-disputes.md)) during epoch e+3's opening days; **measured settlement at e+3** Housekeeping via §6's `settle_cohort → compute_settlement` path. `settle_cohort(e, batch)` processes ≤ 100 (market, position-total) items per call, cursor-resumable; settlement completeness is a precondition for cohort reap; reap is a precondition for pruning a snapshot out of the retained window.

**`epoch.horizon_k`'s ceiling is derived, not chosen (normative; SQ-496 resolution, 2026-07-25).** The cohort decomposition above is arithmetic, not illustration. A cohort created at epoch `e` measures through `e + k`, settles at `e + k + 1` and is removed only then, so it is non-terminal for `k + 2` epochs; one cohort forms per epoch, so steady state holds exactly `k + 2` live cohorts against I-21's cap of 4. The admissible horizon is therefore `k ≤ MAX_NON_TERMINAL_COHORTS − 2 = 2`, and [13](./13-parameters.md) §1's ceiling for `epoch.horizon_k` is that derivation — a **kernel** ceiling, not META-amendable metadata, because raising it is not a tuning choice but a permanent structural over-subscription: at `k = 3` the steady-state demand is `k + 2 = 5` concurrent cohorts against a cap of 4, so the fifth cannot be admitted and `qualify` fails `TooManyCohorts` in **one epoch out of every `k + 2`, forever**. The loss is unrecoverable rather than merely recurring, because a cohort is per-epoch: the proposals a failed epoch turns away cannot join a later cohort — they take T6 once and then T26 — so the chain permanently forfeits a fixed fraction of its measurement capacity with no crank or governance action able to clear it. That this was reachable through a lawful amendment inside the key's own published bounds is what makes the ceiling normative here rather than a note in the parameter table.

**Correction (2026-07-31).** The paragraph above previously read "within a few epochs **every** `qualify` fails `TooManyCohorts` permanently, with no proposal able to enter measurement again". That is false, and executing the cohort machine is what showed it: one cohort retires each epoch once the window fills, so at `k = 3` admission succeeds in four epochs out of five, indefinitely — the failures recur with period `k + 2` rather than accumulating into a halt. The error was reading a permanently saturated *live count* as permanent *admission failure*, which the count cannot distinguish. The ceiling is unaffected — a permanent 1-in-5 forfeit of measurement capacity is decisive on its own — but the failure mode is stated correctly here so no implementation looks for a total halt that never arrives, or treats its absence as evidence that `k = 3` is safe. `reference-model`'s `lifecycle` module records admission per epoch (`CohortRun.admission_rate`, `failure_period`) precisely so the two cannot be conflated again.

The ceiling is also what makes the **"≤ 2 concurrent frozen MetricSpec versions"** bound true. Cohorts freeze their spec version at qualification (I-16) and an activation boundary can leave adjacent cohorts on different versions, so the number of distinct frozen versions consuming one measurement epoch is bounded by the number of cohorts consuming it, which is `k`. Every `× 2` in the derived storage bounds — [02](./02-integration-contract.md) §7.2's `MAX_ROUNDS = 16 × 4 × 2`, [07](./07-oracle-and-disputes.md) §7's per-version aggregates — rests on `k = 2` and on nothing else. An implementation that relaxes this ceiling MUST re-derive all of them; that coupling is the reason the ceiling is stated in this document, which owns `k`, rather than only where the bounds are consumed.
...
**The bound is 20 epochs, and the storage bound is 20 × (`epoch.horizon_k` + 1) records (normative; 2026-07-27).** The retained window above is stated — correctly — in *epochs*, and so is [13](./13-parameters.md) §4's `Snapshots` row. But `Snapshots` is keyed `(epoch, spec_version)` and capacity is enforced against the **record** count, so the epoch bound has to carry §3.3's version multiplicity to mean the same thing: `MAX_SNAPSHOTS = 20 × (2 + 1) = 60`. It did not, and the two rules then disagreed. The consequence was not a capacity nuisance but a permanent wedge: across a MetricSpec activation boundary the epoch legitimately needs both the in-flight cohort's frozen-version record (I-16) and the active-version one, which is more lawful records than slots — so a signed caller could spend the epoch's single spare slot on the cohort record before the keeper wrote the deadline-advancing one, after which `SnapshotDeadline` never advances, §4.8's dead-man latches, and the frozen epoch clock freezes the very prune cutoff that would have released the slot. §4.8 (SQ-254) already names that class: a cause "no crank can clear … (no origin can clear the flag)". **The multiplier is `k + 1`, not `k`.** The admissible set below is the epoch's frozen versions **∪ its own active version**, and the active version need not be a member of the frozen set: the cohorts measuring epoch `e` were created at `e − 1` and `e − 2` (§3.3), so they carry the versions active *then*, and a version activating at `e` itself is a lawful third. Two `register_spec` calls activating in consecutive epochs reach that state through the ordinary governance path, with no attacker and no cadence rule forbidding it. Dropping the active version from the union instead is not available: it is the only version `note_snapshot_recorded` will advance the deadline on, so refusing it *is* the wedge.

The retained **epoch** window is unchanged, so the epoch-keyed bounds derived from it — gate-breach flags, and [13](./13-parameters.md) §4's 21-epoch shared prefix index — keep their values; only the record bound moves. `MaxSnapshots` is a metadata constant the frontend reads, so the change is carried as [02](./02-integration-contract.md) contract **v16**.

Admission is bound with it: `record_snapshot` MUST accept only a version in the epoch's **admissible set** — the epoch's active spec (§4.6) plus every version a live cohort froze for it (I-16) — and not merely any version *activated* by the epoch. The two differ precisely in the versions no consumer will ever read, which is the difference an attacker was spending the spare slot on. `pallet-registry` already binds the identical projection for filings; `record_daily_gate` takes the stricter rule (§4.7).
...
**The exclusion is broad enough; its *enforcement* was not (normative; 2026-08-01, D-20).** "The protocol's own markets" already covers a **hosted external book** ([16](./16-hosted-question-service.md)) — such a book *is* one of the protocol's markets, and only its economic ownership is external, so no reading of this sentence admits it. What was missing is structure: `MetricSpec` carries a source *class* and an opaque `formula_ref` and **no provenance**, `source_matches_pillar` distinguishes only on-chain/relay/attested, and while unknown on-chain metric ids fail closed, **attested values are consumed generically by id** — so the class check cannot see where a number came from. Three requirements follow, and they bind now rather than when hosted books ship: (1) the exclusion reads on **every price, quote, TWAP, volume, liquidity measure or derivative of any Bleavit-hosted book, across every ledger instance and every client**; (2) `register_spec` MUST **reject** a component bound to a hosted book, and metric ids MUST carry runtime-owned provenance rather than a self-declared class — a self-declared source class is not an authorization; (3) a CI dependency/storage-read firewall MUST prevent welfare producers from reading either market instance, on the I-24 lint's pattern. Registration and snapshot attempts naming a service-book TWAP MUST fail **even when source class, `Δs_max`, oracle seats and weights are otherwise valid** — that is the regression test this paragraph owes.
...
| | Relay inclusion/finality `F` (admissible class; **not registered in v1** — §4.3.2) | `1 − clamp(median(relay_parent_gap − target)/Λ_max, 0, 1)` | relay-derived from `PersistedValidationData.relay_parent_number` (the relay best/anchor; `relay_parent_gap` = its advancement across parachain blocks) — verified 2026-07-20 (SQ-282) as the only relay signal on stable2606: the relay **GRANDPA finalized head is not** exposed to the parachain runtime (a relay client-level property, absent from relay runtime storage / validation data / the relay state proof), so `F` is an inclusion/liveness measure, not a GRANDPA-finality one | carry-last-valid + flag | hard to fake upward |
| | Collator concentration `D_eff` | phase-capped, §4.5 | on-chain | — | key-splitting — invulnerable-era value pinned to registry entities |
| **C_onchain** (weighted geo, §4.4) | XCM health `X` (0.25) | v1 (protocol-authored local counters only, [09](./09-execution-upgrades-and-rollout.md) §6.4, SQ-113): locally-accepted protocol sends ÷ (accepted + local send failures + reserve-probe timeouts) over the window — signed exits, client report pushes and sends caused by inbound XCM bypass these counters by construction (I-36); remote delivery/execution outcomes are not runtime-readable on stable2606 (XCM v5, re-checked at the D-19 line move), so X is partial by construction and the R fail-static fallback carries the flag | on-chain counters | no protocol traffic ⇒ 1 (absence of failure) | protocol-send failure alarms ops; public traffic cannot dilute the failure denominator |
...
| | Weight headroom `H` (0.15) | `H` maps the mean of the separately accumulated primary/system `block_utilization(PrimaryUsed, max_block)` samples with the original physical formula `clamp((1 − mean)/(1 − WEIGHT_HEADROOM_TARGET_UTILIZATION), 0, 1)`; the public component remains named `H` | on-chain | no sampled block ⇒ absent, never 1 | spam remains physically observable in the total diagnostic, while only primary/system work drives `H`; the external quota refuses overflow |
| | *(H, external tenancy)* | **Normative from D-20:** the finalization sample still records `block_weight().total()`, so **every** successful external dispatch contributes to the physical/diagnostic record and no import lint can close a resource-consumption channel. The primary sample excludes externally admitted work before applying the **same** two-dimensional `max` reduction against the physical `max_block`; it does not change coordinates or subtract the total. External work is admitted against a separate hard quota in both dimensions, `PrimaryUsed` includes system overhead, and full top-level post-dispatch accounting (nested ledger work, failures, refunds, base-extrinsic weight). **Simple subtraction of service weight is forbidden** — it lets external calls fill blocks while `H` reports full health, which is worse than the exposure it removes. External capacity MUST NOT borrow the primary reservation in any way that can exclude a later primary call. See [16](./16-hosted-question-service.md) §8.5; proven by PT-10 | on-chain | — | two properties that bound the exposure and are easy to overstate: `H` **clamps to 1** at physical mean utilization ≤ 40 %, so external work moves nothing below the target; and `0.15` is a **geometric-product exponent**, not a 15 % linear share of `W` |
...
| | *(P pillar, external tenancy)* | **Normative from D-20, and it binds before the producers exist.** The P-pillar components (fees paid, qualified users, settled value) are global-looking measures whose producers are **unimplemented**. Every one of them MUST exclude hosted-question-service activity — external trading fees, external users and external settled value — or external activity raises Bleavit's own `P` and therefore `W`. Specifying the exclusion now is the whole point: retrofitting it after the producers are written means retrofitting it after external activity has already inflated the measure. Primary `P`/`W` invariance under arbitrary service activity is part of PT-10 | on-chain | — | the exclusion is on the **producer**, not on the aggregation: a component that counted external activity and was then down-weighted would still be measuring the wrong thing |
...
**The denominator is a sum of per-block deltas, not an endpoint difference (normative).** Endpoint advancement is under-determined at both ends and loses outages at window boundaries: for healthy relay parents `r, r+1, …, r+N−1` the endpoint difference is `N−1` against a numerator of `N`, a one-block window divides by zero, and an outage whose catch-up jump lands on a window's *first* block is attributed to neither window. So each block contributes `relay_parent(this block) − relay_parent(previous block)` — and *previous* crosses the window boundary, meaning the last block of the preceding window is the baseline for the first block of this one. The denominator is the sum of those deltas over the window's blocks. Every relay slot is then charged to exactly one window, no boundary loses an outage, and a one-block window is well-defined. **Genesis / missing baseline:** where no predecessor block exists, the first block contributes a delta of 1 — the nominal cadence — so the very first window is scored as healthy on its opening block rather than dividing by zero or inheriting an arbitrary jump.

**`F` — `target` is derived; `Λ_max` is a values judgement and ships `[VERIFY]`.** The `F` row names both and neither is a [13](./13-parameters.md) key.

- `target = 1`, derived. One parachain block per relay slot is the nominal cadence, so a median gap of 1 is a healthy chain and must score `F = 1`.
- **`Λ_max` — `[VERIFY]`, and `F` is fail-closed until it is evidenced.** An earlier revision of this section set `Λ_max = kernel::DEAD_MAN_RELAY_BLOCKS` on the reasoning that `F` should reach 0 where §4.8 declares the chain stalled. **That derivation is withdrawn: it is false, and not merely off by one.** `F` aggregates by **median**, and a median is a central statistic that cannot see the tail the dead-man reacts to. The dead-man latches on *any single* gap at or above its threshold, so a window holding one catastrophic outage among otherwise healthy blocks has median gap 1 and scores `F = 1` — a perfect inclusion score while the dead-man latch is active. No choice of `Λ_max` repairs that, because the disagreement is in the aggregation, not the scale.

**What the three liveness signals actually divide between them (normative).** The false coupling above came from conflating them, so the division is stated here. §4.8's **dead-man** owns the *tail*: a single gap wide enough to mean the chain has stopped. **`U`** owns the *count*: every relay slot the window failed to fill, whether in one outage or spread thin — which is why `U`'s denominator sums per-block deltas and loses nothing at a boundary. **`F`** owns *sustained* degradation: a median gap above 1 means the chain is persistently including at a fraction of cadence, which neither of the other two prices. A tail outage is therefore correctly invisible to `F` and correctly visible to `U` and the dead-man.

That reading is what makes the median the right statistic — and it is also why `Λ_max` has no anchor. It is the median gap at which sustained inclusion counts as fully failed, and nothing in this repository derives it: not a kernel constant, not an existing key, not published calibration evidence. **Both** error directions are unsafe, which is unusual and decisive: too large and `F` stays near 1 through every operationally meaningful degradation, so a persistently degraded chain never trips the `S` gate and gate markets settle "no breach" against real liveness loss; too small and ordinary measurement noise in a short window zeroes `F`, VOIDing cohorts and arming the guardian's `suspend_on_gate` against a healthy chain. There is no direction in which to err safely.

**`F` is therefore an admissible component that v1 does not register (normative).** Tagging `Λ_max` `[VERIFY]` and leaving `F` "unavailable" is *not* a viable alternative, and the reason is mechanical: a registered component with no value makes `record_snapshot` refuse, so a genesis spec set containing `F` would be unable to record any snapshot at all, ever — welfare would be wedged rather than degraded. Registration is the choice point, so the choice is made there. `F` keeps its frozen `MetricId` 11 (ids are never reused), keeps its formula and source column above, and becomes registrable as a later `MetricSpec` version the moment Phase-2 shadow-run data evidences `Λ_max` — the ordinary append path this section already describes, with the `≥ current + 2` activation lead protecting in-flight cohorts (I-16).

Dropping it costs nothing structurally, and that is a property of `S` specifically: `S` is a **`min`** and carries no weights, so an absent component simply leaves the minimum — §4.4 states exactly this for the §10 drop case, and the intra-pillar weight-sum rule applies to `C`, `P` and `A`, never to `S`. Contrast the `P` pillar, which cannot drop a component without renormalizing. Nor is liveness left unpriced: **`U`** prices every unfilled slot in the window whatever its distribution, and §4.8's **dead-man** owns the tail. `F` was the third signal on that axis and the only one whose scale nothing derives — so v1 ships `S = min(U, D_eff)` and `S_daily = min(U^{day}, D_eff^{day})`, on two signals that are fully derived, rather than on three where one is a guess.
...
For the external-service partition, use the separately accumulated `PrimaryUsed` but keep the physical
coordinate system: `PrimaryUsed` contains every top-level primary dispatch's full actual charge,
including nested ledger work, failed calls, refunds and base-extrinsic weight, plus residual
system/inherent overhead. The shared helper normalizes `ref_time` and `proof_size` against the same
physical `max_block`, rounds each quotient up on the 1e9 grid, and takes their maximum. External work
is admitted against a separate hard quota in both dimensions and cannot borrow the primary
reservation. The total sample remains separately recorded for diagnostics — every successful
external dispatch is still physically visible there — but it is not subtracted to manufacture `H`.

The window value is the arithmetic mean of those primary physical-coordinate samples, mapped by the
original affine clamp:

```
H = clamp( (1 − mean(u_block)) /
           (1 − WEIGHT_HEADROOM_TARGET_UTILIZATION), 0, 1 )
```

This is deliberately the pre-partition integer path. With zero external usage, `PrimaryUsed` equals
the total physical used weight and `H` is **bit-identical for every input on the 1e9 grid**, not only
at the target and full-utilization crossovers. The implementation must not materialize separately
rounded `scale_primary` and `target_primary` constants. If a future representation cannot preserve
that identity, any deviation MUST be downward (understated health), never an overstatement. The
shared helper also makes proof-size-bound blocks and ref-time-bound blocks select the same dimension
for both utilization and H; there is no second coordinate selector to drift. `H` clamps to 1 at or
below the 40 % target and reaches 0 at physical 100 %. `WEIGHT_HEADROOM_TARGET_UTILIZATION` remains a
kernel constant, not a [13](./13-parameters.md) key.
...
**Weights live in the MetricSpec.** The `MetricSpec` record gains normative fields: `pillar ∈ {S, C_onchain, C_attested, P, A}`, `weight: FixedU64`, `epsilon_floor: FixedU64`, and — for a milestone component of the A pillar — `target` (the divisor of §4.3's `min(1, points ÷ target)`, frozen per version so a live cohort's milestones can never be retroactively renormalized; [07](./07-oracle-and-disputes.md) §7), and — for every component — `delta_s_max_bps` (the documented maximum single-epoch settlement impact `Δs_max` that [07](./07-oracle-and-disputes.md) §6.3's admission rule is stated against), alongside the existing `{ id, formula_ref, units, repr, source class, cadence, normalization rule, sanity bounds, missing-data rule, gaming vectors + min-cost estimate, challenge procedure, version, activation_epoch ≥ current + 2, in-flight rule }`. Registering a spec whose pillar weights do not sum to 1, missing the gaming-vector section, or declaring a milestone component with no positive `target`, MUST be rejected.

**`Δs_max` — units, bounds and scope (normative; SQ-341 resolution, 2026-07-25).** The field is **mandatory on every component** and carried in **basis points** of settlement score, matching [07](./07-oracle-and-disputes.md) §6.1's rule that the §6.3 comparison is evaluated in basis-point space (a documented 0.10 shift in `s` is `1_000`). It MUST satisfy `0 < delta_s_max_bps ≤ 10_000` for a component whose source class is **attested**, and `register_spec` MUST reject a spec that violates that bound — a zero would assert that a component cannot move settlement at all, which no attested component can truthfully claim, and a value above 10,000 exceeds the range of `s` itself. For source classes 1–3 the field is recorded but **not** bound-checked and not consumed: those components run no §5 game, so there is no bond ladder to collateralize them against. An earlier revision of [07](./07-oracle-and-disputes.md) §6.3 cited this field as living at "05 §12.4-equivalent"; this document has no §12, and the citation was a stale BE-numbering artifact. **§4.4 is its home.** Open cohorts always settle on their creation-time spec version, weights included (I-16). **Activation is implicit and scheduled, not a second transaction (normative; SQ-80):** registering a spec version *is* the activation decision — it stores an immutable `activation_epoch` that makes the version **eligible** at that epoch, with no further call. There is no pending/active state pair and no `welfare.activate_spec`; the two-call surface formerly in [doc 06](./06-governance-and-guardians.md) §2.1/§3.2 is struck. The reason is liveness: a two-phase model creates a registered-but-never-activated limbo that one missed governance step can leave permanently pending, whereas a stored future epoch cannot fail to arrive. At each epoch the **active** version is the *unique* registered version with the latest fully-live `activation_epoch`; registration does not forbid two versions sharing an `activation_epoch`, and if several tie then **no version is active** — the fail-closed direction. Eligibility is therefore scheduled, activeness is resolved per epoch, and the two MUST NOT be conflated. At genesis the lead is one epoch (activation at epoch 1) because epoch 0 is the pre-launch sentinel; post-genesis the `≥ current + 2` lead applies.
...
**Settlement-time renormalization when a component is dropped (normative; SQ-493 resolution, 2026-07-26).** [07](./07-oracle-and-disputes.md) §10 drops a component that was flagged in two consecutive epochs of a cohort's measurement window and recomputes that cohort's `W` "with weights renormalized". §10 owns *which* components are dropped and *when*; this section owns the arithmetic, and it is the quotient `C_daily` already specifies: within each affected group the surviving weights become `w_j / Σ_surviving w`, on the same Q64.64 grid and with the same evaluation order and rounding as every other term. The flagged set, together with the `I_e` and tunables the recompute must replay the epoch against, is welfare-internal state keyed by `(EpochId, MetricSpecVersion)` — exactly one record per snapshot, written with it and retired with it, and machine-checked in `try-state` in both directions ([15](./15-invariants-and-testing.md) §1). It is deliberately not a `Snapshots` field: that map is a frozen [02](./02-integration-contract.md) §7.4 frontend read surface and none of this is frontend data. `S` is a `min` and carries no weights, so a drop simply removes a value from it. Three further rules make the degenerate cases determinate.

1. **A group that the drop would empty declines the drop.** `S` and `C` enter `W` only through the gates `g(S)`, `g(C)`. A gate has no weight to renormalize away, and both a `min` over nothing and a weighted product over nothing evaluate to the *most favourable* value 1 — so an emptied gate group would convert total unavailability into a perfect score, and dropping the factor outright would treat an unmeasurable security pillar as no constraint at all. Such a group therefore keeps every component it had, and §10's VOID path — a different mechanism with a different trigger (§4.7) — remains the only response to gate-input failure. This case is unreachable for a conforming v1 spec set, since [07](./07-oracle-and-disputes.md) §2's split leaves no gate input attested; it is specified so an implementation cannot improvise.
2. **An emptied `P` or `A` renormalizes one level up.** Both are weighted terms of the single `GeoComposite`, so the same rule applies to *its* weight vector: the surviving pillar's weight becomes `w / w = 1` and the composite is that pillar's value alone. With `w_P + w_A = 1` enforced, a fully-dropped `A` gives `W = g(S) · g(C) · P`. `W` is then measured on what could be measured, rather than on `A`'s stale carried values — and never on a fabricated `A = 1`. If *both* would be emptied there is nothing to renormalize onto and the composite declines the drop as in (1).
3. **A degenerate composite is evaluated exactly, not through the transcendental round-trip.** Where exactly one term of a weighted geometric group participates and its (possibly renormalized) weight is exactly 1, the group's value **is** that term's value: every other term contributed a factor of exactly 1. This has to be stated because the round-trip does not reproduce it, and not for the reason one would expect. `x^1` evaluated as `exp2(1 · log2 x)` must truncate an irrational `log2 x` to the 64.64 exponent grid under the determinism discipline below, and that truncation alone lands one 1e-9 ulp below `x` whenever `x` lies on the 1e9 grid — in **both** conforming implementations, at any working precision. So it is neither a precision artifact nor a divergence either implementation could catch against the other; it is simply wrong. The flooring rule exists to pin how an *inexact* product rounds, not to inject error into an exponentiation with no arithmetic content. (Contrast `s` in rule 4 below, where the residual genuinely is a finite-precision effect and the ≥ 256-bit reference model's exact square root is the arbiter.) The case is not hypothetical: renormalization is what makes a weight of exactly 1 reachable in production, by dropping one of two equally-weighted components.

**Determinism discipline (normative):**
1. All component values are `FixedU64` (1e9) in [0,1] before aggregation.
2. Weighted geometric terms are evaluated in **ascending `MetricId` order** as `y = exp2(Σ w_i · log2(max(x_i, ε)))` in the 64.64 `futarchy-fixed` representation ([doc 04](./04-markets-and-pricing.md) §fixed-point); each `w_i · log2(·)` product is rounded toward −∞ at 64.64 before summation, subject to the exactness rule for a degenerate single-term group above.
...
**A truncated or empty authorship window makes `D_eff` unavailable (normative; 2026-07-27).** `D_eff` reads a *distribution*, not a count, so a bounded authorship collection that dropped an author is not conservative here: if the retained authors are earlier low-count ones and a newly rotated author then produces most of the remaining blocks, the retained distribution looks uniform while the real one is concentrated, and `D_eff` is **overstated**. A conforming runtime MUST therefore latch truncation per window and report concentration as unavailable rather than compute it from a partial set. The same disposition covers an **empty** window: HHI over an empty author set would score a perfect `D_eff` from absent data, the fabricate-health-from-absence error §4.3.2 already rules out for `H`. Counts are unaffected — `K` and `U` remain readable from a truncated window, because for a count the drop is genuinely pessimistic. (13 §4's bound row records the storage bound; this section owns the behaviour.)
...
**`log1p` evaluation and the degenerate range (normative; SQ-502 resolution, 2026-07-26).** §4.4's determinism discipline requires two conforming implementations to agree bit-for-bit, and the steps above leave three choices open that decide whether they do. All three are fixed here.

1. **Grid.** `log1p(x) = ln(1 + x)` is evaluated in the 64.64 `futarchy-fixed` representation ([doc 04](./04-markets-and-pricing.md) §fixed-point) — the value enters through the same `Q64(x) = floor(x · 2^64 / 10^9)` conversion §4.4 fixes for the weighted-log products — and the result is floored back onto the `FixedU64` 1e9 grid **before** the min–max map. No new logarithm is introduced: `ln` is the existing 64.64 primitive. The flooring is not cosmetic. Carried at 64.64 into the min–max, the logarithm's ≤ 2 ulp residual is divided by `(hi − lo)` and can therefore be amplified without bound as the winsorized range narrows, which is exactly the regime a quiet series occupies; floored to the 1e9 grid first, the min–max quotient is exact integer arithmetic and identical in both implementations by construction.
2. **What is transformed.** `log1p` is applied to the series **and to its p5/p95 bounds**, and the min–max endpoints are the transformed bounds. The transform is monotone, so winsorizing before or after it is the same clipping; the order stated above (winsorize, then `log1p`, then min–max) is the normative evaluation order.
3. **A zero-width range fails closed.** If `hi ≤ lo` after the transform — an unmoved series, or two distinct raw bounds that collapse onto one grid point under `log1p` — the component has **no** normalized value and the computation MUST refuse. It MUST NOT resolve to 1.0, 0.5, or 0. A series with no observed spread carries no information, and the adopt-favourable convention would hand a pillar a perfect component computed from absent data — the opposite of the status-quo default (G-1). Refusal propagates as a missing component under that component's missing-data rule.

**Which series are heavy-tailed is §4.3's table, not a tunable.** The transform is declared per component where the component is defined: in the v1 set exactly one carries it, `P` fees at `N(log1p(fees_USDC))` (MetricId 20). Admitting a second heavy-tailed component is an amendment to §4.3's table first, and a MetricSpec registration second.
...
**The measurable day set (normative; SQ-181 resolution, 2026-07-26; scheduled reading ruled 2026-07-27).** A day index is a measurement window only if the epoch actually contained it, read against the epoch's **scheduled** length — not against the span a paused epoch physically occupied. An epoch's measurable days are its **whole** days — a legal `epoch.length` need not be a day multiple, and a trailing partial day is not a completed cadence slot — **floored at one**, so a sub-day epoch still requires one recorded observation rather than passing vacuously. `record_daily_gate` MUST refuse a day outside that set. It MUST NOT resolve such a day to any value.

The rule is needed because without it each component improvises a different answer, and they do not agree. For a day the epoch never had, `X` reads 1 under its "no traffic ⇒ 1" rule, `R` refuses (it already derives exactly this whole-day range), and `K` reads 0 because no collator authored in a day that never elapsed. A keeper could therefore record a non-existent day and drive `C_daily` down out of components that were never measured — an adopt-adverse breach manufactured from absent data, the mirror of the fail-destructive direction the zero-sample rule above closes. Refusing the day *removes* the disagreement instead of harmonizing three defaults, and it is the only disposition that does not require declaring one of the three readings correct for a window that does not exist.

**"Actually contained" means scheduled, not elapsed (normative).** §4.8's dead-man keeps the day index advancing while the clock is paused, so a paused epoch can physically span more days than it was scheduled to hold, and the two readings diverge exactly when the chain is least healthy. The scheduled reading is normative because its error direction is safe: it can only **refuse** a day that physically elapsed, costing coverage — which §4.7 already tolerates, since it declares no expected-day count — whereas the elapsed reading would **admit** days created by the pause itself, and those days are measured against components that were not running. Admitting them manufactures a breach out of a liveness failure the S pillar and the dead-man already price, which is the fail-destructive direction G-1 exists to exclude. A conforming runtime therefore derives the set from the epoch's retained schedule, and refuses when that schedule is no longer retained.

This governs the day **domain**, not coverage. The paragraph below deliberately declares no expected-day count per epoch and this introduces none: one recorded day inside the measurable set still satisfies the rule. `MAX_DAILY_GATE_SAMPLES` remains a *storage* bound on the breach bitmap and is not the semantic bound — an epoch may be shorter than it, and a day below it can still be outside the epoch.
...
1. **Timing.** Cohort e settles at e+3 Housekeeping (frozen constant), after Snapshot(e+2) finalizes and every consumed attested component's challenge window has closed ([doc 07](./07-oracle-and-disputes.md)). Any component not challenge-closed by its **`OracleSettleDeadline`** — the start of epoch `m+1`'s Housekeeping for measurement epoch `m`, d20 ([doc 07](./07-oracle-and-disputes.md) §11; consolidated in [doc 13](./13-parameters.md) §2) — settles neutrally, so cohort settlement always proceeds in its scheduled Housekeeping and never waits on a live dispute. For the binding case `m = e+2` that is this cohort's own settlement Housekeeping, which is why the deadline is driven from every epoch entry point that can be the first to cross it. Stated as `m+1` rather than "the consuming epoch" deliberately: doc 07 §11(1) owns the definition and parameterizes it on `m` alone, so an `m` whose `e = m-2` cohort never formed keeps the same deadline instead of inheriting a later one from its surviving consumer.
...
- **Every other affected proposal** — the epoch's non-terminal pre-Executed proposals — takes T20: the path MUST terminally archive it with state `Rejected(ProcessHold)`, MUST emit `ProposalForceRejected`, and MUST move any market-backed vault to `Voided`. It MUST write `DecisionOutcome::Reject(ProcessHold)` **only where no decision is recorded**; an already-recorded outcome is retained (SQ-319 resolution, 2026-07-24 — see §2.1).

...
| | *(H, external tenancy)* | **Normative from D-20:** `H` samples `block_weight().total()` at finalization, so **every** successful external dispatch contributes and no import lint can close a resource-consumption channel. `H` MUST therefore be computed as `H_primary` against a **reserved primary capacity**, with hard per-block external quotas in **both** ref-time and proof-size, a separately accumulated `PrimaryUsed` including system overhead, and full top-level post-dispatch accounting (nested ledger work, failures, refunds, base-extrinsic weight). **Simple subtraction of service weight is forbidden** — it lets external calls fill blocks while `H` reports full health, which is worse than the exposure it removes. External capacity MUST NOT borrow the primary reservation in any way that can exclude a later primary call. See [16](./16-hosted-question-service.md) §8.5; proven by PT-10 | on-chain | — | two properties that bound the exposure and are easy to overstate: `H` **clamps to 1** at mean utilization ≤ 40 %, so external work moves nothing below the target; and `0.15` is a **geometric-product exponent**, not a 15 % linear share of `W` |

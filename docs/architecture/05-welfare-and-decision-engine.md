...
That reading is what makes the median the right statistic — and it is also why `Λ_max` has no anchor. It is the median gap at which sustained inclusion counts as fully failed, and nothing in this repository derives it: not a kernel constant, not an existing key, not published calibration evidence. Its error direction is **unsafe** — too large and `F` stays near 1 through every operationally meaningful degradation, so a persistently degraded chain never trips the `S` gate and gate markets settle "no breach" against real liveness loss. Under [AGENTS.md](../../AGENTS.md) R-2 a value with that shape ships `[VERIFY]`-tagged with its consumer fail-closed rather than picked, so **until `Λ_max` is evidenced, `F` is *unavailable***: a registered `F` component yields no value, and the snapshot crank fails status-quo-safe exactly as it does for `R` before the reserve probe arms and for `E` while `collator.bond` is unseeded. Evidencing it is Phase-2 shadow-run work, alongside [13](./13-parameters.md)'s other sim-gated rows.

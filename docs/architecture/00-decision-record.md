...
- Books per proposal ≤ **6** (2 decision + 4 gate); Baseline books share the primary capacities rather than carrying a separate count cap. `MaxLiveMarkets = 196` bounds primary books without a terminal latch and their live POL obligations; the physical `Markets` map is partitioned into `MaxStoredMarkets = 2,240` primary rows and `MaxStoredExternalMarkets = 15,744` external rows, for `MaxAllStoredMarkets = 17,984`. The external partition derives from fastest lawful service throughput across the one-year archive ceiling and cannot borrow from the primary partition. All PoV/storage budgets are re-derived from this one table in `13-parameters.md`.
...
- `IntakeQueue = 64`; `MaxLiveProposals = 32`; books/proposal ≤ 6; primary `MaxLiveMarkets = 196` / `MaxStoredMarkets = 2,240`; external `MaxLiveExternalMarkets = 128` / `MaxStoredExternalMarkets = 15,744`; shared `MaxAllStoredMarkets = 17,984`; `RecentCohortSummaries` ring = 32; `MaxPositionsPerAccount = 64` (protocol accounts exempt); Positions deposit 0.1 USDC; key order `(PositionId, AccountId)`.

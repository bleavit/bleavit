...
- **Contract v5 (2026-07-19, G0 universal TREASURY gating):** §4's Treasury class-floor semantics now use the single gate-bearing floor for **every** TREASURY proposal; `INTEGRATION_CONTRACT_VERSION` bumped 4 → 5. This is a **pre-genesis** revision with no migration, jointly signed off by the user (owner for both backend and frontend sides under R-1). Detail in 02 §13 version history.
...
- **Amended 2026-07-19 (G0 Phase-0 simulation response):** every TREASURY proposal, irrespective of ask/NAV ratio, carries the existing four-book Survival/Security gate set. The former ≤ 1% NAV exception is removed; CODE and META remain gated and PARAM remains the sole static-classification market-bearing class. No gate representation or parameter changes: the existing `(S,C)×(adopt,reject)` books and `gate.*`/`pol.b_gate` values are reused. User-authorized after the gate-attacking simulation found zero profitable exploits for gated classes; owning text: 04 §1/§9, 05 §5, 08 §3–§5.

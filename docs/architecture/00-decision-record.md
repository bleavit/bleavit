...
- **Phase 3–4 trading accuracy rewards (added 2026-08-10)**: the 100M-VIT incentive pot delivers its trading share through the bounded PARAM-origin `fund_trading_rewards` path, one epoch budget at a time and under a lifetime authorization bound — the same shape B20 gave the community pot, and for the same reason. The program pays **realized forecast accuracy** at `rwd.rate` = **0.25 %**, mints nothing, and never touches the conditional ledger. Score, earning cap and forfeit are all computed in **USDC** against a held USDC bond, so no VIT price enters the anti-farm invariant (D-18); conversion to VIT happens once, at claim. Forfeited bond goes to `INSURANCE`. Rate derivation, bond rules and failure behaviour: [08](08-treasury-and-economics.md) §2.6.

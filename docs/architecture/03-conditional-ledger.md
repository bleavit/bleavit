...
| `BaselineVaults: map EpochIndex → BaselineVaultInfo` | §2.2 | Open/market-backed entries share `MaxLiveMarkets = 196`; creation is reached only through the bonded proposal/POL-funded epoch path, and terminal entries are permissionlessly swept after the archive delay. Only a `Settled` vault is reapable (§5.4), so finite retention rests on **every** opened Baseline vault reaching `Settled` — §5.2's two neutral paths close the cases (voided cohort, and an epoch whose cohort never formed) that the measured e+3 settlement does not reach |

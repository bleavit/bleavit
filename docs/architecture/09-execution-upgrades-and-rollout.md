...
| Aspect | v1 rule |
|---|---|
| XCM version | **XCM v5** — latest stable on `stable2603` (verified 2026-07-16, PLAN V-15); version negotiation enabled |
...
| Asset mappings | USDC (Asset Hub location ↔ local `ForeignAssets` id, pinned per D-17: `{parents: 1, X3(Parachain(1000), PalletInstance(50), GeneralIndex(1337))}` — asset index 1337 verified as Circle-native sufficient USDC, 2026-07-16, PLAN V-16) and DOT (parent) only; unknown assets refused |
| Reserve model | Asset Hub is reserve for USDC; relay/AH for DOT; **teleports disabled** — scope: this chain's executor trusts no inbound or outbound teleport (`IsTeleporter = ()`); programs this chain *authors* may instruct the **relay** to run its canonical DOT teleport to a system chain (the §4 Coretime funding leg, PLAN V-18), which executes on the relay under the relay's own trust rules and creates no teleport trust here (clarified 2026-07-16) |

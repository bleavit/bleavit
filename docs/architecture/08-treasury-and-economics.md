...
- **USDC** balance **above `T_ins` is surplus and MUST overflow to `MAIN` automatically**, in the same transaction as the inflow that created it. No governance act, no proposal slot, no crank. The inflows this actually reaches are the **USDC** ones: the 10 % intake-bond slashes of §7 (`prop.bond` = 1k / 5k+0.5%·Ask / 25k / 50k USDC), which land far above `T_ins` and pass straight through, and every other USDC arrival not matched by a residue increment. Those slashes therefore keep their full deterrent effect — the offender still forfeits — while the forfeited value becomes spendable NAV instead of dead reserve. The VIT bond slashes of [06](06-governance-and-guardians.md) §4 and §7 are outside this rule per the asset-scope paragraph above; they were cited here in the rule's first form and that citation was wrong.
...
- **(b) The custody return.** The book account's surviving inventory — complete LONG+SHORT (resp. YES+NO) sets on the realized branch, worth exactly one branch-USDC per set — MUST be redeemed to real USDC and returned to the funding line: `POL` for decision and gate books, `POL_BASELINE` for the Baseline book. Only the divergence loss stays spent. This is the inverse of step 2 and it is what makes POL a **revolving** balance rather than a per-epoch expense.
...
|---|---|---|---|
| **1,145,562** (derivable lines only — a floor, §10.1) | `H` = 190,926,921 (`V` = 381,853,842) | **138,855,943** (416,567,828) | 78,732,751 (456,649,955) |
...
**Read against §10.3's capacity table, the crossover sits at the protocol's own design point rather than far beyond it.** At `C` = 1.146M and the central τ = 3, `V*` = 138.9M — **1.17×** the 5 × TREASURY saturated slate and **0.64×** the 5 × CODE one. A mature chain running treasury- and code-class decisions at the depth its own security parameters assume is at or just short of break-even at today's fee rates, with no rate change. A chain running PARAM-only slates at the bare `dec.v_min` floor earns 151k–367k against ≥ 1.146M — a **3.1–7.6× shortfall** — and no rate inside the [13](13-parameters.md) registry closes it.
...
| `C` = 2.954M (**the pre-E1 implementation** — the derivable base plus the leak below) | 1.3 yr | 3.8 yr | 6.9 yr |

...
> regenerated 2026-08-07 at integration contract v28, picking up F1's FE-P1 resolution in
> §11.3: the `polkadot-api/pjs-signer` export names are pinned rather than tagged
> `[VERIFY]`, FE-P6's remaining question is narrowed to the device half, and PAPI is
> recorded as signing `CheckMetadataHash` in **mode 0** on every in-app path. The previous
> pass picked up the §11.8 operator reads that v28 freezes and the two defects freezing
> them exposed: the authorized upgrade hash is read from `System.AuthorizedUpgrade`, not
> from `parachain-system` storage, and `ExecutionGuard.PendingUpgrade` carries a fourth
> field, `target_spec_version`. For upload to Claude Design. If this copy and the source

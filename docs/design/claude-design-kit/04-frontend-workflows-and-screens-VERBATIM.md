...
> regenerated 2026-08-06 at integration contract v28, picking up the §11.8 operator reads
> that v28 freezes and the two defects freezing them exposed: the authorized upgrade hash
> is read from `System.AuthorizedUpgrade`, not from `parachain-system` storage, and
> `ExecutionGuard.PendingUpgrade` carries a fourth field, `target_spec_version`,

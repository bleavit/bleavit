...
> `NavView.insurance_target`, and §11.8.2 gains one row per `PlaybookTrigger` variant
> naming the frozen item that establishes it — three of which the prose had never named.
> The registry call names, the welfare-snapshot call and its key, and the guardian propose
> flow are trued to the runtime in the same pass. §11.8.6's O-8 row is bound to
> `Epoch.CohortSchedules`, whose `specs` field answers *"the versions live cohorts froze
> for `epoch`"* — the row had carried no clause at all, so it claimed coverage of a
> precondition nothing could evaluate. From **F1** in the same regeneration: §11.3's
> signer bullet drops its `[VERIFY — FE-P1]` tag and pins the PAPI export names.
> For upload to Claude Design. If this copy
...
The system's most privileged actors get a specified signing tool. Approval aggregation lives inside `pallet-guardian` (not `pallet-multisig`); the console reads guardian storage (`Guardian.Members`, ≤ 7; `Guardian.Allowances`; `Guardian.PendingActions`; `Guardian.Approvals` for the m-of-7 tally; and `Guardian.PlaybookRegistered` and `Guardian.ActivePlaybooks` for the two activation refusals below — item names frozen in [02](02-integration-contract.md) §7.4) and provides:
...
| Approve flow | `guardian.approve_action(action_id)` (call name per [06](06-governance-and-guardians.md) §5.1); preconditions: caller is a member; action pending and unexpired; caller has not already approved; the approval renders the **exact enumerated call batch** being approved (playbooks are preimage-committed enumerated batches — decoded and displayed, never summarized away). An approval that would be the **dispatching** one carries two further preconditions, because the runtime evaluates them only then: for an `ActivatePlaybook` action, `Guardian.PlaybookRegistered(id)` is `true` (else `PlaybookNotRegistered`), and the named playbook is admissible against `Guardian.ActivePlaybooks` — presence blocks **`PB-LEDGER-FREEZE` alone** (`PlaybookAlreadyActive`), while every other playbook re-activates in place without consuming a slot, and an absent id additionally needs a free slot of the six ([06](06-governance-and-guardians.md) §6.2) |
| Propose flow | **one call, five argument sets** — `guardian.propose_action(power, justification_hash)`, where `power` is the `GuardianPower` enum's `PauseIntake { until }`, `DelayOnce { pid }`, `ForceRerun { pid }`, `ActivatePlaybook { id, trigger, expiry, target }` or `SuspendOnGate` ([06](06-governance-and-guardians.md) §5.1/§5.2). Preconditions: allowance remaining for the power (allowance meters displayed) — **the meter is a pair** and each half is a separate read: `Guardian.Allowances` carries the *used* counters, and the limits are the four metadata constants frozen at contract v30 ([02](02-integration-contract.md) §9) — `Guardian::DelayOnceAllowancePerEpoch`, `Guardian::ForceRerunAllowancePerEpoch`, `Guardian::PauseIntakeAllowance` and `Guardian::PauseIntakeAllowanceWindowEpochs` — never a client literal (§13 rule 4); a playbook activation additionally requires the **`trigger` it names** to be verifiably active at B′ — the trigger read is part of the precondition row, and the table below binds each variant to the item that establishes it |
...
**An allowance meter is a used counter and a limit, and until contract v30 only the counter was published (SQ-733).** `Guardian.Allowances` stores `AllowanceState { delay_used_this_epoch, force_rerun_used_this_epoch, pause_window_start, pause_used_in_window }` — every field a *use*, none a bound — while [13](13-parameters.md) §1's entrenched `grd.bond` / allowances row recorded the limits as deliberately not metadata-exposed. So the propose row above required a comparison whose right-hand side no surface carried, and the only client that could render the meter was one that supplied the numbers itself. [15](15-invariants-and-testing.md) §2's INV-FE-1 forbids exactly that twice: an unsourced value must not be represented as verified, and must not satisfy a precondition. [02](02-integration-contract.md) §9 now freezes the four limits as metadata constants and 13 §1's note is corrected. Two rules follow. The client MUST read **both** halves and MUST NOT fall back to a literal if either read fails — an unreadable limit is an unevaluated precondition, which INV-FE-12 refuses with the reason shown rather than passing. And the `pause_intake` meter is windowed rather than per-epoch: remaining is `PauseIntakeAllowance − pause_used_in_window`, and the window is live only while `current_epoch − pause_window_start < PauseIntakeAllowanceWindowEpochs`; outside it the counter is stale and reads as a full allowance, so a client comparing `pause_used_in_window` against the limit without the window shows an exhausted meter for a power that is available.

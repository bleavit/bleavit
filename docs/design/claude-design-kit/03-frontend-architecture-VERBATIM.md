...
> cannot host the shipped worker. The §12 gate table records five gates closed and
> three half-closed. The previous pass picked up the §2.3 verb ruling: the
...
**Five of the eleven gates are now closed and three more are half-closed, and every one of those resolutions came from a pinned source or a published one rather than from the experiment its own row named.** That pattern is the finding worth carrying, because it cuts against the row: FE-P2 said *"wire-level test with a lying mock peer"*, FE-P6 said *"hardware test"*, FE-P5 said *"probe"*, FE-P1 said *"2-day spike against Paseo"*, FE-P7 said *"ar.io testnet dry run"* — and in each case the load-bearing half was a name, a default, a branch, or an authorization rule that someone had already published. An experiment column is a **plan**, not a statement about what the answer requires, and a milestone note that repeats it as "needs hardware" converts a plan into a blocker. Read the pinned source first. What genuinely remains needs a device lab, a live chain, or a credential: FE-P3's Safari and Android halves, FE-P4's latency/memory/throughput/CPU, FE-P6's blind-signing question, FE-P7's four platform halves, FE-P10's memory and liveness halves, and FE-P11's share matrix.
...
| FE-P9 | Bulletin mirror dry run (deferred D-Bulletin triggers T1–T4) — **the triggers are not stateable from this doc set; raised as SQ-941** | TestNet pipeline run, *once a trigger fires*. No trigger can fire while its definition is missing | secondary mirror only; never canonical. **This gate is not blocked on a testnet.** T1–T4 live only in `FRONTEND_PLAN §31`, which doc 00 superseded and which is not in the repository; the sole surviving pointer is 12 §4.1's parenthetical. The related evidence question, Bulletin Chain mainnet availability ([07](07-oracle-and-disputes.md) §9), is also open. Either restate the triggers here or retire the row |

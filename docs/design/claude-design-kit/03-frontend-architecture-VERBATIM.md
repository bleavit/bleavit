...
> state-root form. The previous regeneration was 2026-08-07 and carried three changes that
...
**The distinction is not an implementer's judgement call, and this list is exhaustive of the chosen side.** Chosen: the trade size, a cost ceiling or proceeds floor, the fee asset, the signing account, the action selected, and the target's id where the user picked it from a chain-read list. **Everything else the payload carries is sourced and MUST be `Finalized<T>`** — nonce, era and mortality anchor, `spec_version`, metadata hash, genesis hash, market and vault ids resolved from chain state, every quote or cost the client recomputes, every balance, phase, rate or flag a precondition reads, and — named explicitly because it is the one an implementer is most likely to take from a wallet or an RPC — **the transaction fee estimate and the fee headroom a precondition checks against**. A fee figure that is not derived from finalized runtime inputs is **advisory only**: it may be displayed as such, and it MUST NOT gate, satisfy, or be presented as the basis of a signature. **FE-P1's fee-estimation half resolved 2026-08-07, and the rule above is now unconditional rather than conservative-pending.** PAPI 2.1.8 answers `getEstimatedFees(from, txOptions?)` from `TransactionPaymentApi_query_info` over a fake-signed extrinsic of the right length, issued as `chainHead.call$(null, …)` with the runtime context taken at `null` as well — and `null` resolves to the **finalized** hash (§4.2, FE-P2). Its own docstring says so: *"Estimate fees against the latest known `finalizedBlock`"*. So the conforming figure is obtainable, the qualifier that softened this clause is withdrawn, and a client has no remaining excuse for a fee taken from a wallet or an RPC. Three properties of that surface are binding on implementers, because each is a way to hold the right API and still get an unsourced number: a `TxOptions.at` passed to `getEstimatedFees` moves only the *fake extrinsic's* mortality anchor and **not** the `query_info` call, which stays on finalized; the returned `PaymentInfo.partial_fee` **excludes the tip**, which is a separate `TxOptions` field the caller sets; and `partial_fee` is denominated in the **native** token even when `ChargeAssetTxPayment` names another fee asset, so the conversion to USDC is the client's own and rides `fee.vit_usdc_rate` (§11.3 of [11](11-frontend-workflows.md)). A value does not become "chosen" because a user typed it: if the client will render it as true of the chain, or evaluate anything against it, it is sourced. **Reclassifying a sourced value as chosen to escape the finalized-state obligation is the failure mode this paragraph exists to foreclose.**
...
**There are no `archive_*` methods, and historical reads at arbitrary depth are unavailable — but the mechanism is not the one this section asserted until FE-P5 resolved (2026-08-07).** The `archive_*` half is confirmed: `smoldot@3.3.2` implements no method of that family, at the tag the lockfile pins. The other half — *"smoldot exposes the `chainHead` JSON-RPC group only"* — was **false**, and is corrected here rather than quietly kept, because a reader who checked would find the legacy group present and conclude this section was merely over-cautious. smoldot implements 63 of the 88 methods its `MethodCall` enum declares, including `state_getMetadata`, `state_call`, `chain_getHeader` and `chain_getBlock`; the remaining 25 answer `-32000 Not implemented in smoldot yet`. `state_getMetadata { hash }` and `state_call { …, hash }` accept **any** hash, at unbounded depth, and resolve it over the network: header fetched and checked by hash, `:code` fetched under a storage proof against that header's own state root, runtime compiled and executed locally. That path is proof-backed exactly as §4.2's `chainHead_v1_call` argument describes, and it carries **no pin and no subscription**.

  **What actually bounds depth is hash acquisition, not state retrieval.** `chain_getBlockHash(height)` returns `null` for every height except `0` and the best block, deliberately, and smoldot's source states the reason: *"While could ask a full node for the block with a specific number, there is absolutely no way to verify the answer of the full node."* A light client therefore cannot enumerate history **by height** at all. From one trusted hash it can walk backwards by `parentHash`, each step hash-verified, at one round trip per block — O(depth), unbatchable. The second bound is peer-side: smoldot's own runtime service records that a block is skipped when peers *"refuse to answer or have already pruned the block"*, which surfaces to a caller as a retryable `-32000` rather than as a wrong answer.

  **Two traps for the §5.2 classifier and for any capability probe.** `rpc_methods` returns the raw macro-generated name list — all 88, including the 25 that are not implemented — so it over-reports and MUST NOT be read as a capability set. And `state_getMetadata` hard-requires `Metadata` runtime-API version `1..=2`, while `state_call("Metadata_metadata_at_version", …)` carries no such requirement: a future runtime declaring `Metadata` v3 breaks the first and not the second.

  Consequences, normative:

- Historical reads at arbitrary depth through the light client **remain unavailable in practice**, and the three-layer model of §6 stands unchanged — but on the corrected grounds above (no verifiable height→hash map; peer pruning), not on an absent method group. Every design element that assumed `eventsAt(hash)` at depth (the old §15.4 loop unbounded, §15.6 backfill past the window, E3's "history continuous") is replaced by that model. Events are state (`System.Events`), and a client that has not walked the parent chain to a block has no verifiable hash with which to ask for it.
- **The §9.3 release-shipped metadata blobs are still required, and FE-P5's fallback is therefore adopted rather than merely available.** Per-era decode needs metadata for a *height range* the client identifies from its own index, and there is no verifiable route from a height to a hash. Retrieving that metadata at depth would first cost one round trip per intervening block.
...
- **Follower transactions**: a follower that needs to sign either proxies preparation/submission through the leader channel or spawns a transient smoldot instance of its own for the duration of the flow. Which is viable is still open on the device halves: **[VERIFY Web Locks behavior on Safari, BroadcastChannel snapshot latency, dual-instance memory on Android — FE-P3; the prototype gates the final choice]**. Until FE-P3 resolves, the transient-second-instance path is assumed.
...
- **SharedWorker is off the table at the pinned versions, and the reason is structural rather than a matrix result (FE-P3's `startFromWorker` half, resolved 2026-08-07).** The gate asked about *"SharedWorker compatibility with `startFromWorker`"*. `startFromWorker` is PAPI's export, not smoldot's — `smoldot@3.3.2` splits the work as `smoldot/worker`'s `run(messagePort: MessagePort)` against a client-side `ClientOptions.portToWorker`. PAPI's wrapper is typed `(worker: Worker, options?) => Client` and calls `worker.onmessage = …` then `worker.postMessage(port1, [port1])`; a `SharedWorker` is not a `Worker` and carries neither member. **The decisive half is the worker-side entry point**: `@polkadot-api/smoldot/worker` is three lines over the *bare global* `postMessage` and `onmessage`, which exist in a `DedicatedWorkerGlobalScope` and not in a `SharedWorkerGlobalScope` — where a connection arrives through `onconnect` instead. So the shipped module cannot run in a SharedWorker at all. A SharedWorker deployment would need a bespoke worker entry driving `smoldot/worker`'s `run()` with the port from `onconnect`, which is buildable and is **not** what this client uses. SharedWorker therefore stays a possible desktop optimization, never a correctness dependency, and the 2× Android budget line above is unaffected.
...
Ingested event subset, per-era metadata decode discipline (decode with the producing runtime's metadata; undecodable rows stored raw, "N events pending decoder", never guessed) — carried forward. **Releases ship metadata blobs for supported historical spec_versions, bounded per §9.3.** That is unconditional, not a fallback for a case that may not arise: FE-P5 resolved 2026-08-07 (§4.2), and what it found is that per-era decode needs metadata for a *height range* while a light client has no verifiable route from a height to a hash. The blobs are the only route, so the "where unavailable" qualifier this sentence used to carry is withdrawn.
...
| **every row carries the full four-valued `origin` (+ `providerId`), including `priceSamples` and the candle tables** | the row's origin is the *whole* mitigation §2.3 offers for chart data, and INV-FE-15 requires it "to the pixel". A two-valued provenance forces the writer to guess which third party a `provider` row came from, and the guess badges an opt-in indexer's row as protocol-funded layer-2 data. On a chart row the two provider values mean **derived by this client from that provider's movements**, never a row the provider served — §8.5.2 (SQ-983), which is why the badge still names the third party |
...
Per-provider health probe on enable + every 10 min **for a source that is not `Disabled`** — a disabled source is not contacted at all (§8.5.3, [11](11-frontend-workflows.md) E10); `Healthy → Slow → Failing → Disabled(auto, reason)`; auto-disable on sampling mismatch. All-providers-down ⇒ the default (provider-less) behavior with the standard incomplete-history explainer.
...
**`Failing` serves only where the healthy series exists.** The clause above licenses `Failing` to serve on a stated ground — *"one timeout in a healthy series cannot ratchet the ladder"* — and that ground is a precondition rather than a decoration. A source accepted a moment ago whose **first** probe timed out has no series at all, and letting it serve would take it from `unprobed`, which serves nothing, to `Failing`, which serves: failing to answer would buy it exactly the eligibility that not being asked withheld. That is the same non-monotonicity the paragraph above rules out for the wrong-chain case, with silence rather than a wrong answer as the trigger, and it survived the first correction because each rule is right in isolation. So `Failing` serves once the source has answered at least one probe, and does not before. The consecutive counter runs either way, so a source that never answers still reaches `Disabled` on schedule and still carries its reason.
...
**Historical metadata comes from the block being decoded, and FE-P5's bound does not reach this tool.** §6.5's discipline binds the producer exactly as it binds the client: decode with the producing runtime's metadata, never guess. The producer obtains that metadata with `archive_v1_call` at each block whose events it decodes. FE-P5 resolved 2026-08-07 (§4.2) and its *scope* plainly excludes this tool — §12 states it as retrievability *"via light client"*. What that resolution found is that depth is bounded by **hash acquisition** rather than by state retrieval: a light client cannot verify a full node's height→hash answer, so `chain_getBlockHash(height)` returns `null` and history is walkable only by parent-chaining. A Node command-line tool talking to an archive node does not inherit that bound, because `archive_v1_hashByHeight` answers exactly the question smoldot refuses — which is the same reason §8.5.1 above requires the producer to resolve each height to a hash before reading it.
...
**A provider serves observed facts; this client owns every derivation. Ruled 2026-08-07 (SQ-983).** One format means the document §8.2 defines, which carries `vaults`, `ops`, `balances` and `coverage` and no chart row — while §7's schema requires `priceSamples` and both candle tables to carry a four-valued `origin` that includes `indexer`. Read together those said no indexer can supply the rows one of them says it supplies. The `'indexer'` origin is correct and stays; what it means is fixed here: **a chart row whose `origin` is `snapshot` or `indexer` is one this client derived locally from that provider's `ops`**, not one the provider served. Nothing else changes — the badge still names the third party the data came from, which is the whole mitigation §2.3 offers for chart data, and INV-FE-15's *"to the pixel"* is satisfied by the same `providerId` beside it.

The alternative was to extend §8.2's document with the sample and candle tables, and it costs more than it looks. Aggregates are a **function** of the movements beside them, so a document carrying both can contradict itself and §8.4 would owe a fourth screen to catch it. Worse, reproduce-by-anyone would start depending on two producers agreeing on a *bucketing* — boundaries, tie-breaks, the empty-bucket rule — rather than on a canonical serialization of what they observed, and two honest producers that bucketed differently would fail `FE-PROV-004` on every pair. Deriving locally has neither problem, and it is not a new mechanism: §9.1 already consumes chain-wide `Traded` into the candle aggregates as it is scanned and never stores a trade row, so the client already builds candles from a movement stream. This ruling says the provider path uses the path that exists.

**What it does not yet settle, stated here rather than discovered later.** `bleavit.snapshot.v1`'s movement alphabet is [03](03-conditional-ledger.md) §5's complete-set operations — `split`, `merge`, `transfer`, `redeem` — and not one of them carries a price. So there is currently nothing in a provider's `ops` to derive a chart row *from*, and this ruling narrows the contradiction to a single question rather than removing it: either §8.2's document gains an **observed trade** movement, which is an observed fact and therefore untouched by the argument above against carrying aggregates, or §7 drops the two provider values from its chart-row `origin` and says provider history is movements only. **SQ-989** owns it, and until it is ruled no surface renders a provider-origin chart.

**A page owes canonical form and §8.2's ordering rules. It does not owe the conservation replay.** This has to be said, because one format could otherwise be read as one set of obligations, and that reading makes ranged reads impossible rather than merely strict. §8.4's replay starts every holding, supply and escrow at zero and requires non-negativity at each step. A `split` mints from escrow and is self-contained at any span, but a `merge`, `transfer` or `redeem` of a position created in an earlier block replays negative — so a page covering blocks 15 to 19 of a real history is inadmissible, and a conforming operator could serve only spans reaching back to the origin of every position they touch. §8.4 already resolves this and the split is its own: it assigns the internal-consistency screens to **snapshots**, and gives **live indexers** *"1-in-16-page row re-verification"* instead.

**Which of §8.4's three screens that drops, exactly.** §8.4 names them as one phrase — *"monotone coverage, event↔derived-row agreement, conservation-identity replay"* — and read as a block it takes all three away, which is wrong in a way that is easy to miss: the argument above reaches only two of them, and the third is what makes a page's coverage claim true about the page's own contents.

- **The conservation replay's non-negativity drops.** It requires non-negativity at every step from a zero start, and a page that opens mid-history legitimately consumes positions minted before it. This is the impossibility above, and it is the part of the screen that carries it. Its **referential** checks stay, and the distinction is the same one this paragraph is making one level down: *does the document declare the vault this movement names, and does that vault have this branch* are questions about the page alone, a mid-history page answers them, and nothing else asks them — §8.4's sampling re-verifies `balances` and never reads a movement.
- **The event↔derived-row agreement drops too, and for its own reason.** Separating the two matters, because a first draft of this section dropped both under the replay's argument and that argument is not true of this screen: it compares `balances` against the fold of the document's own movements, which is a *self*-check and would survive a restriction perfectly well. What removes it is the ruling below — on a page, `balances` is **not** the fold.
- **Monotone coverage stays.** It is entirely internal to the document — the ranges are ordered and non-overlapping, they sit inside the declared span, and every movement falls inside a covered range — and a page over blocks 15 to 19 satisfies all three without difficulty. It is also load-bearing here rather than merely harmless: a client establishes its coverage from the union of the lists the pages carried, so a page whose movements sit outside its own declared coverage would hand that client movements at blocks its coverage does not claim. §8.2's ordering rules, which a page owes in as many words, already require the list's form; this is the same requirement applied to the movements beside it.

**A page's `balances` are read from state, not folded from the page.** Dropping a screen leaves the field it screened unconstrained, and an unconstrained field a client ingests is worse than a checked one — so this has to be said in the same breath. Two consumers fix the answer between them and neither is a choice made here. §8.5.1 already has a producer *"publish balances read independently from state at the range's last block"*, and the route table above already assigns this route to **§8.4's 1-in-16-page sampling**, which re-verifies rows against chain state *"where the referenced object still exists"*. Sampling is the entire verification budget §8.4 gives a live indexer, and it has nothing to check against unless those rows are real current holdings. A folded `balances` would be a restatement of the `ops` beside it — re-verifying it against the chain would compare a mid-history fold with a current balance and disagree on every honest page, which turns §8.4's one live control into a source of false mismatches that auto-disable honest operators.

So a page carries the movements it observed **and** the state those movements land in, and the pair is what makes the route useful: the `ops` are what layer-3 ingests, and the `balances` are what sampling audits. The line is therefore not *internal consistency versus not*. It is **a page is checked against itself, and against the chain — never against a history it does not carry.**
...
An earlier draft of this table said `/chain` also served *"the compatibility check before any range read"*, and no section defines such a check. The phrase is removed, and what replaces it is a ruling rather than a gap.

**The two version fields are read and displayed. They never refuse. Ruled 2026-08-07 (SQ-982).** The question looked open because §6.3 forbids the analogous comparison for a *snapshot* on a reason that does not carry — §6.4 assigns snapshots history predating the current runtime, so comparing would refuse every deep snapshot after the first upgrade, whereas an indexer's reported binding is current. But the comparison fails for a different and simpler reason: **it would protect nothing.** A page is a §8.2 document — self-describing JSON whose amounts are canonical decimal strings — so a client decodes one without runtime metadata, and a version difference therefore cannot make a page unreadable. That is precisely what separates this from §13.1's exact-equality rule, which governs the three **handoff** formats: each of those describes one block and is decoded *against runtime metadata*, so there a version the client cannot decode makes the document unreadable and refusing is the only honest answer. Refusing here would deny history over a difference that changes nothing about the client's ability to read it.

So both fields travel, both are rendered as an advisory line, and neither gates a read — the same treatment `admitSnapshot` already gives a snapshot's versions. The probe's answer condition stays the `genesisHash` comparison alone, now because that is the whole of what matters rather than pending a ruling.

**`GET /range` pages ascending, and that does not fight §6.4's backfill. Ruled 2026-08-07 (SQ-984).** A page covers *"some prefix of the requested span"*, which is ascending from `from`, while §6.4 walks history **newest to oldest in 1,000-block chunks** — and the two never meet, because they order different things. §6.4 orders the *spans a client asks for*; this route orders the *pages inside one span*. A client asking for the newest chunk first and reading it front to back gets its early-stopping from the chunk sequence, which is where §6.4 puts it. The residual cost is bounded by one chunk, which is what choosing a chunk size is for, and admitting a direction parameter would buy that back at the price of a second paging mode every operator must implement and every client must test.
...
**A failed read does not advance §8.3's ladder — but a *disqualifying* one does.** The ladder is probe-driven, and letting ordinary read failures ratchet it would add a second, busier path to auto-disable: a provider serving a heavy screen would disable faster than an idle one, and a mechanism whose trigger rate depends on how much the user is reading is not the liveness signal §8.3 describes. So a `503`, a timeout and a malformed body stay off it.

That rule alone leaves a hole, and the hole is invisible from either side of it. With the read path contributing **nothing**, a source that answers `GET /chain` and fails every `GET /range` can never be disabled by anything at all: its probes keep succeeding, so the ladder holds it `Healthy`, and §8.4's sampling never runs because no rows arrive to sample. Worse, a page carrying a binding for **another chain** is precisely the evidence §8.5.3 makes terminal, discarded for no reason except that it arrived on the read path rather than the probe path. So the split follows the line §8.5.3 already draws: **a liveness failure never reaches the ladder from a read, and a correctness finding always does.** A page whose binding names another chain disqualifies the source, and disqualification disables at once.
...
`metadataCache` (historical SCALE metadata for per-era decode; **measured 0.15 MB gz** per blob — DEFLATE level 9 over the committed 473,749 B `metadata.scale`, against the "~1–2 MB" this section previously assumed): bounded at **≤ 8 blobs / ≤ 15 MB desktop, ≤ 3 blobs / ≤ 3.75 MB mobile**. Those byte bounds are §9.2's metadata share exactly; the previous 16 MB / 6 MB caps **exceeded their own share** in both cases (SQ-557), which is a bound that cannot bind. At the measured blob size the **count** limit is what actually binds and the byte limit is headroom against metadata growth — eight blobs are ~1.2 MB. This cell read 0.14 MB until `app/tools/check-artifact-budget.ts` re-measured it: the blob is 148,175 B, which is 0.15 MB at the two decimals this figure is published to (it read 146,946 B over a 469,581 B blob before contract v28's six frozen operator reads, 147,008 B over a 470,546 B blob before v29's two runtime-API methods, two `NavView` appends and two frozen storage items, and 147,926 B over a 472,998 B blob before v30's four guardian allowance constants; the rounded gz figure has not moved through any of the three, so §9.4's metadata row is unchanged — which is the point of publishing a *rounded* measurement against a bound the count actually binds), and a *measured* value has to round rather than truncate because §9.4's metadata row is derived from it. LRU-evicted; the current and next-authorized runtime's metadata are pinned non-evictable. Eviction of a blob needed by old undecoded rows is acceptable: those rows already carry the raw-bytes "pending decoder" state (§6.5) and re-fetch/re-ship paths exist (FE-P5). Release-shipped blobs (the FE-P5 fallback) count against the same bound **and against the §9.4 bundle row**, which they previously did not have.
...
| smoldot WASM (worker, lazy) | ≤ 3.5 MB gz — **measured 2.21 MiB gz** for `smoldot@3.3.2` (FE-P4's artifact half, resolved). Only the gzip figure is toolchain-dependent, so this cell stays a bound and never becomes an equality check | size gate + lazy load |
...
| Release-shipped historical metadata (gz, lazy) | ≤ 1.5 MB combined — §9.3's 8-blob cache bound × the measured 0.15 MB blob, rounded up for metadata growth. The release cannot ship more blobs than the cache admits | size gate — `app/tools/check-artifact-budget.ts`, over the committed per-`spec_version` blobs, against **both** bounds: this combined size and §9.3's blob count. The row was titled *"fallback metadata"* until FE-P5 resolved (§4.2, §6.5). These blobs are **required**, so a budget row calling them a fallback invited a release to skip them |
...
| Code | Raised when | Recovery |
|---|---|---|
| `FE-FEE-001` | The fee rate is unusable: `fee.vit_usdc_rate` is unreadable, non-positive, carries a non-positive scale, or sits outside the [0.1×, 10×] band the constitution holds it to ([11](11-frontend-workflows.md) §11.3) — **and a rate outside its own bounds is refused rather than clamped**, because clamping transacts at a price the chain says is impossible | No fee figure and no signature. Retry at a later finalized block; an out-of-band rate needs a PARAM amendment, not a client retry |
| `FE-FEE-002` | A **tip** is requested. `TransactionPaymentApi_query_info`'s `partial_fee` **excludes the tip** (§2.3), so a headroom computed from it understates what the account must hold by exactly the tip — and that lands as a rejection after signing. Whether headroom must include the tip or render as a separate line is an open question ([11](11-frontend-workflows.md) §11.3 and §11.5 are both silent), so the client refuses rather than understates | Submit untipped. The refusal expires when the headroom question is ruled, not by a client change |
...
- **The `FutarchyApi` cross-check is per domain.** Every `FutarchyApi` result on the transaction path is re-derived from direct storage reads (§4.2): `service_positions()` cross-checks against the `ServiceLedger` prefix and `account_positions()` against instance `()`. Satisfying one with the other's keys would make the check vacuous in exactly the case it exists for. **FE-P2 resolved positively on 2026-08-05, so this is no longer a conservative posture pending a gate** — §4.2 retains the control on a different rationale, as defence against a client misreading an aggregate API's semantics or decoding it wrongly, which proof verification says nothing about. Per domain for the same reason: two instances are two semantics, and reading one through the other's keys is exactly that class of mistake.

---

## 12. Prototype experiments and open questions (carried forward, updated)

The [VERIFY]/prototype epistemics of the reviewed design are retained deliberately — honesty over polish. Conservative assumptions still in force: dedicated workers + leader election with transient second instances (FE-P3's device halves); hash routing regardless of FE-P7's finding; backfill arithmetic at 20 blk/s until FE-P4 measures reality; clipboard and file primary with Share a fallback (FE-P11's matrix half). FE-P2's cross-check of runtime-API results is no longer one of them — that gate resolved positively (§4.2), and the cross-check is retained on a *different* rationale rather than as a pending assumption.

**Four of the twelve gates are now closed, six more are half-closed, and every one of those resolutions came from a pinned source or a published one rather than from the experiment its own row named.** Closed: **FE-P1, FE-P2, FE-P5, FE-P8**. Half-closed: **FE-P3, FE-P4, FE-P6, FE-P7, FE-P10, FE-P11**. Untouched: **FE-P9** and **FE-P12**, which need a live chain and a live archive node respectively. That pattern is the finding worth carrying, because it cuts against the row: FE-P2 said *"wire-level test with a lying mock peer"*, FE-P6 said *"hardware test"*, FE-P5 said *"probe"*, FE-P1 said *"2-day spike against Paseo"*, FE-P7 said *"ar.io testnet dry run"* — and in each case the load-bearing half was a name, a default, a branch, or an authorization rule that someone had already published. An experiment column is a **plan**, not a statement about what the answer requires, and a milestone note that repeats it as "needs hardware" converts a plan into a blocker. Read the pinned source first. **Read the repository's own history too**: FE-P9 stood as blocked on triggers this document called unstateable, and they were in a superseded file `git show` prints in full — recording a gate as unresolvable by assumption is the same defect as resolving one by assumption. What genuinely remains needs a device lab, a live chain, or a credential: FE-P3's Safari and Android halves, FE-P4's latency/memory/throughput/CPU, FE-P6's blind-signing question, FE-P7's four platform halves, FE-P9's T1–T3, FE-P10's memory and liveness halves, FE-P11's share matrix, and FE-P12's archive-node depth.

| ID | Question ([VERIFY] owner) | Experiment | Gate |
|---|---|---|---|
| FE-P1 | ~~Exact PAPI 2.x surface: at-block query options, best-block observable, compatibility API, fee estimation, pjs-signer exports, CheckMetadataHash handling~~ **RESOLVED 2026-08-07 — all six halves, §2.3 and §5.2** | Read from `polkadot-api@2.1.8` and `@polkadot-api/pjs-signer@0.7.3` in the lockfile-pinned tree, the method V-82/V-87 established. The spike column said *"against Paseo"*, and no part of the question needed a chain: every half is a name, a default, or a branch in shipped source | **All six answered.** *At-block*: `PullOptions.at` is `"best" \| "finalized" \| string` and **defaults to finalized**; `watchValue` takes `"best" \| "finalized"`, and `watchEntries` defaults to finalized too but types **only** `{at: "best"}` — so its default is safe while the sole option a caller can spell is the unsafe one. *Best-block*: `bestBlocks$` / `getBestBlocks()` (best-first, finalized-last) beside `finalizedBlock$` / `blocks$` — and `bestBlocks$` **may skip blocks** by its own docstring, so it is not an ingest feed. *Compat*: V-87. *Fee*: `getEstimatedFees` is `TransactionPaymentApi_query_info` at the **finalized** block (§2.3). *pjs-signer*: V-96. *`CheckMetadataHash`*: PAPI signs **mode 0** unconditionally (`both(zero, zero)`), overridable only through `customSignedExtensions` — which is what B21 will need |
...
| FE-P3 | Web Locks leader election + BroadcastChannel snapshot latency on Safari; ~~SharedWorker with `startFromWorker`~~ **(that half RESOLVED 2026-08-07, negative — §4.4)**; follower-tx path (proxy vs transient instance); dual-instance memory on Android | matrix spike on device lab, for the three device halves that remain | gates §4.4 final design; §9.4 dual-instance budget line. **SharedWorker cannot host the shipped worker entry point**: `@polkadot-api/smoldot/worker` uses the bare global `postMessage`/`onmessage` of a `DedicatedWorkerGlobalScope`, and PAPI's `startFromWorker` is typed `(worker: Worker, …)`. §4.4 keeps SharedWorker as an optimization only. Safari Web Locks, BroadcastChannel latency and Android dual-instance memory still need the lab |
| FE-P4 | Real sync latency, ~~smoldot artifact size~~ **(measured, V-88/V-300)** + per-instance memory, ingest throughput (the 20 blk/s anchor), measured proof sizes, mobile CPU | instrumented testnet runs on device lab | release-gate values for §9.4. The artifact half is a standing gate (`pnpm run check:budget`) at **2.21 MiB gz** against ≤ 3.5 MB. Note that only the *gzip* figure is toolchain-dependent — the raw and brotli totals reproduce byte-for-byte across zlib versions, so this cell is a budget and must never become an equality check. Everything else here needs the lab |
| FE-P5 | ~~Historical metadata retrievability via light client at depth~~ **RESOLVED 2026-08-07 — §4.2; the fallback is adopted** | Read from smoldot's Rust source at `npm-smoldot-v3.3.2`, the tag the lockfile pins — now at `paritytech/smoldot` (commit `5fe9121f81a58454542ac69a44c4d73f00f30283`, 2026-07-24), because `smol-dot/smoldot` redirects to an archive where the tag no longer resolves. V-89 correctly found this unanswerable from the JS package and named the Rust source as the way in | **Structurally bounded, and not where the question assumed.** Metadata at depth **is** retrievable — `state_getMetadata`/`state_call` take any hash, unbounded, proof-verified. What is refused is `chain_getBlockHash(height)`, for every height but genesis and best, because a light client cannot verify a full node's height→hash answer. So backfill **by height is impossible**, and by parent-chaining costs one round trip per block. §9.3's shipped blobs are therefore **required**, not merely a fallback. Peer retention is the remaining live-probe question, and it is a property of our own nodes rather than of smoldot |
| FE-P6 | ~~Ledger Generic App + metadata-hash flow for a custom chain~~ — **the load-bearing half RESOLVED 2026-08-05 (V-122, SQ-594), and the blocker is the chain rather than the device.** What still needs hardware is narrower and is stated as such: whether a given Ledger app falls back to **blind signing** when a chain offers no digest | hardware test, for the blind-signing half alone. The metadata-hash half needed no device: `frame-metadata-hash-extension-0.10.0` takes `RUNTIME_METADATA` from `option_env!("RUNTIME_METADATA_HASH")`, a **compile-time** variable `substrate-wasm-builder` sets only when metadata-hash generation is enabled | wallet support tier ([11](11-frontend-workflows.md)). **This runtime does not generate its digest as built**, so `implicit()` returns `CannotLookup` for mode `Enabled` and every mode-1 transaction is rejected today — ruled **SQ-594** and routed to milestone **B21**. A hardware test run before B21 would have returned a confusing result: the device behaving correctly and the chain refusing what it produced. The client is already correct, because `packages/signing` grants no `metadata-hash` capability and has no function that could |
| FE-P7 | **ANT n-of-m controller capability — RESOLVED 2026-08-07, negative ([12](12-release-and-operations.md) §4.2, SQ-940)**; two-pass manifest flow; undername immutability practice; resolver endpoints; manifest fallback behavior | ar.io testnet dry run, for the four halves that remain. The n-of-m half needed no testnet: it is a property of the ANT program's authorization code, which is published | **Native ANT n-of-m does not exist, so 12 §4.2's primary option is unavailable and its FROST fallback is now the path.** An ANT controller list is an **access-control list**, so it is 1-of-m: adding controllers widens custody instead of splitting it, which is the opposite of what D-16 asks. **And the platform moved** — the AO implementation was archived 2026-06-09 and ArNS now runs as Solana programs, which changes what a "controller address" and a threshold ceremony are. Raised as **SQ-940**; the four remaining halves must be re-asked against the current platform before any dry run is designed |
...
| FE-P9 | Bulletin mirror dry run: chunked upload of `dist/` (size against the 8 MiB per-transaction limit), CID determinism across re-uploads, Bitswap and gateway retrieval latency, renewal flow, and dotNS registration with n-of-m control. **The D-Bulletin triggers are quoted in the gate column; SQ-941 is re-scoped to the question that actually remains** | TestNet run of the full pipeline against the Bulletin authorization faucet, once T1–T3 hold | secondary mirror only; never canonical. **The triggers were never missing, and an earlier draft of this row said they were.** `FRONTEND_PLAN.md` §31 states them, at commit `6657f438`, which is an ancestor of `main` — doc 00 superseded that file rather than deleting it, and a superseded document is still readable. Verbatim: *"(T1) Bulletin Chain live on Polkadot mainnet; (T2) a non-OpenGov authorization path (PoP or delegated) shipped, or an OpenGov authorization obtained; (T3) retention/renewal mechanics stable and priced; (T4) `tools/verify-release` extended to check CID ↔ release-hash equality"*. **T4 is this repository's own work rather than an external event**, which is why §31 gated the dry run on T1–T3 and gated *publication* on all four. T1–T3 each need a live chain, an authorization path, or a published price, so this row is externally blocked on infrastructure after all. What it was never blocked on is a definition. One question remains and it is about status rather than content: doc 00 says the component documents supersede `FRONTEND_PLAN.md`, while [12](12-release-and-operations.md) §4.1 cites §31 as live, so whether these triggers bind normatively by reference or are only the historical statement needs a ruling (**SQ-941**). The related evidence question, Bulletin Chain mainnet availability ([07](07-oracle-and-disputes.md) §9), is open and is exactly T1 |
| FE-P10 | Multi-MB Wasm extrinsic submission through smoldot/PAPI in-browser: ~~transaction-pool/gossip size limits, peer banning on oversized transactions~~ **(both RESOLVED 2026-08-07, positive)**, mobile memory headroom ([11](11-frontend-workflows.md) §11.8.4) | instrumented testnet submission of a real runtime artifact (extends FE-P4), for the memory and liveness halves | gates the FE-15 upgrade-crank submission tier; the in-browser fetch + hash-verify path ships regardless, with the operator-CLI handoff as fallback. **No size ceiling in any pinned source refuses this extrinsic.** `apply_authorized_upgrade` is `DispatchClass::Operational`, so its length ceiling is the full 5,242,880 B rather than the 3,932,160 B normal share, and a ~1.89 MB runtime image encodes to about 1,894,266 B — 36 % of it. The peer cap is 16 MiB, and smoldot's own 1 MiB notification cap is **inbound only** and never binds, because a substrate node does not send transactions to a `Light`-role peer. **Nothing bans a peer for size.** What remains is memory and liveness: the JSON-RPC boundary carries the payload as a hex string across a Worker copy, local `validate_transaction` runs the runtime over it, and whether a peer opens a transactions substream to a light client at all is peer behavior no constant answers |
| FE-P11 | Handoff transports (§13.4): `navigator.share({ files })` availability across the browser/OS matrix, and ~~confirmation that Web Share, the async Clipboard API and File System Access are **not** governed by CSP `connect-src`~~ **(that half RESOLVED 2026-08-04, positive)**. The second half is load-bearing for §13's INV-FE-6 claim, so it is verified rather than assumed | the CSP half was read off the primary specifications: fetch directives are enforced at one hook inside Main Fetch, so a capability that never enters Fetch is unreachable by `connect-src`. The matrix test for `navigator.share({ files })` still needs the device lab | decides whether Share ships as a primary or fallback transport. Conservative assumption in force until the matrix half resolves: **clipboard and file are primary, Share is a fallback**. The CSP half was the outcome to look for first and it is positive, so §13 is not blocked — with the consequence that `check:handoff-network` is the **only** mechanical control over egress on a handoff path, because CSP structurally cannot see these transports |
| FE-P12 | **Does a conforming archive node answer `Metadata_metadata_at_version` at arbitrary depth?** §8.5.1 requires `tools/snapshot` to decode each block's events with the producing runtime's metadata (§6.5), fetched by `archive_v1_call` at that block. That the node still *has* the state is an operational expectation about how an operator configured it, not something the JSON-RPC interface specification guarantees — and §8.5.1 declines to rest on *"an archive node retains historical state by definition"*, which would be resolving a `[VERIFY]` by assumption. Distinct from **FE-P5**, which asked the same question of the **light client** and resolved 2026-08-07: that answer turned on hash acquisition — no verifiable height→hash map — and an archive node serving `archive_v1_hashByHeight` is not subject to it, which is why this row survives FE-P5's closure rather than closing with it | call `archive_v1_call` for `Metadata_metadata_at_version` at descending depths against a public Polkadot and Paseo archive node, and record the depth at which each stops answering | **fails closed already, so it blocks nothing**: the producer refuses any block it cannot decode rather than emitting it raw, which keeps an incomplete op set out of a published document. A negative result bounds how deep a snapshot can reach, and that bound belongs in §9.3's release-shipped blob policy rather than in the tool |

...
| `crates/` | scaffold + N5 | `futarchy-primitives` (M1) and `futarchy-fixed` (M2) live here; Track A's per-pallet **frame-free functional cores** land here too as `crates/<name>-core/` (`no_std`, no `frame` deps — the differential oracle + WASM/auditor port). N5 adds `question-service-core`, the frame-free hosted-question lifecycle/report/manipulation-bound/attestor-median core; N7 supplies its FRAME shell |

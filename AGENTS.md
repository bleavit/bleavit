...
| `runtime-api/` | B2 + N7 + N9 + N13 done | `futarchy-runtime-api`: the frozen **13-method** `FutarchyApi` (02 §3–§4a). N9's trailing `ClientRecord.delivery_float` activated v22; **N13 appends `service_positions` and activates integration contract v23** (SQ-571) — the canonical client serves external books, so the `ServiceLedger` (`Instance1`) rows are canonical ingest surface and the two domains answer in separate 64-slot vectors because the bound is per instance. The separate monitoring-only `TelemetryApi` is now v4 with isolated service-egress counters and remains outside 02 |

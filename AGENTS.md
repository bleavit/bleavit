...
| `crates/` | scaffold + Track A + N4 + N5 | `futarchy-primitives` (M1), `futarchy-fixed` (M2), Track A's per-pallet frame-free cores, and N4's `client-registry-core`; every `crates/<name>-core/` is `no_std` with no FRAME dependency; `crates/question-service-core/` is the N5 frame-free service core, and `crates/client-registry-core/` the N4 one |
| `pallets/` | Track A + B4 residual + N7 implementation | Production FRAME shells over the frame-free cores; Track N adds the exact-identity, native-bonded `pallet-client-registry` and the two-book `pallet-question-service`. `pallets/inflow-caps` remains the deliberate state-only exception |
| `runtime/` | B1–B12 + B15–B19 + N7 residual + N8 ingress | `runtime/bleavit-runtime` is the real Cumulus parachain runtime (`construct_runtime!`, `impl_runtime_apis!`, `BaseCallFilter = SafetyFilter`). Frozen custom slots now include `Epoch` 61, `ExecutionGuard` 62, `InflowCaps` 63, `TrackOrigins` 64, `ClientRegistry` 65, `QuestionService` 66, and `ServiceLedger` 67. Existing wiring includes six-track governance, live Params consumers, native holds, treasury/XCM posture, generated weights, the 12-method `FutarchyApi`, and N8's exact registered-client XCM conversion/filter intersection |
| `runtime/bleavit-xcm/` | B4 + N8 done | The XCM layer as a runtime-independent library the runtime wires: 09 §6.1 rule-table barrier/assets/trader components, the 07 §8 reserve-probe program + authenticated response router, the 09 §4 coretime-renewal funding leg (verified relay-teleport route), 09 §5.2 inflow-cap adapters, the `pallet_xcm` call classifier, and N8's exact six-position client-ingress barrier plus registered-location origin converter and classifier-derived safe-call filter. The legacy closed instruction allowlist is unchanged; only the exact template reaches `Transact` |
| `runtime-api/` | B2 + N7 done | `futarchy-runtime-api`: the `sp_api::decl_runtime_apis!` declaration of the frozen 12-method `FutarchyApi` (02 §3–§4a) over the view types in `futarchy-primitives`. N7 added `hosted_report`, bumped the API version, and activated **integration contract v21**; `futarchy_primitives::INTEGRATION_CONTRACT_VERSION` and 02 §13 remain the authorities. B13's separate monitoring-only `TelemetryApi` trait stays outside the 02 contract, owned by 12 §6.3 and consumed only by the ops exporters |

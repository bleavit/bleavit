...
| `docs/proposals/` | living | **Pre-implementation designs**, one file per proposal, named `YYYY-MM-DD-<topic>-design.md`. A proposal is a plan and is **never normative** — it records the reasoning, the rejected options and the `docs/architecture/` edits an implementation would have to make under R-1. It stops being the source of truth the moment those edits land. A proposal whose milestone reaches ✅ is history, not status: PLAN.md owns status (R-4) |

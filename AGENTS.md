...
| `runtime-api/` | B2 + N7 + N9 done | `futarchy-runtime-api`: the frozen 12-method `FutarchyApi` (02 §3–§4a) remains unchanged; N9's trailing `ClientRecord.delivery_float` activates **integration contract v22**. The separate monitoring-only `TelemetryApi` is now v4 with isolated service-egress counters and remains outside 02 |

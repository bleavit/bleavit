...
| App (`app/`) | `pnpm -C app install --frozen-lockfile`, then every gate catalogued in `.claude/rules/app-code.md` · *Quality gates for `app/`* — moved out of this always-loaded file 2026-08-06 (38k chars every session, most of them spent where `app/` is never opened). That file loads automatically whenever a session touches `app/**`, which is exactly when these gates bind; they are unchanged and non-optional. Read it before running or changing any `app/` gate |

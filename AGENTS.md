...
| App (`app/`, once scaffolded) | `pnpm -C app lint typecheck test build` · dependency-cruiser firewall clean · the **negative-compilation fixtures** (a corpus of forbidden imports that MUST fail `tsc -b` — 15 §4.8 tests that the app works, never that the firewall *rejects*) · `protocol` differential vs `reference-model/fixtures/vectors.json` · descriptor drift vs each published runtime artifact · the 02 §9 no-literal gate · the hostile-intent corpus rejected by class (one `FE-HANDOFF-*` code each) · the handoff no-network source gate + CSP-allowlist diff · the `MockSigner` release-exclusion assertion · the INV-FE-4 no-infra certification run with providers, RPC **and the handoff surfaces** disabled |
...
| `crates/` | scaffold + Track A + N4 + N5 + N9 | `futarchy-primitives` (M1), `futarchy-fixed` (M2), Track A's per-pallet frame-free cores, N4's `client-registry-core`, N5's `question-service-core`, and N9's `bleavit-client-abi`; the ABI crate fixes the sole report receiver call shared with N10, while every `crates/<name>-core/` stays `no_std` and FRAME-free |
| `pallets/` | Track A + B4 residual + N7/N9 | Production FRAME shells over the frame-free cores; Track N adds the exact-identity, native-bonded `pallet-client-registry`, N9's separately custodied USDC `delivery_float`, and the two-book `pallet-question-service` with best-effort report push. `pallets/inflow-caps` remains the deliberate state-only exception |
| `runtime/` | B1–B12 + B15–B19 + N7 residual + N8 ingress + N9 egress | `runtime/bleavit-runtime` is the real Cumulus parachain runtime (`construct_runtime!`, `impl_runtime_apis!`, `BaseCallFilter = SafetyFilter`). Frozen custom slots now include `Epoch` 61, `ExecutionGuard` 62, `InflowCaps` 63, `TrackOrigins` 64, `ClientRegistry` 65, `QuestionService` 66, and `ServiceLedger` 67. Existing wiring includes six-track governance, live Params consumers, custody, the 12-method `FutarchyApi`, N8's exact registered-client ingress intersection and N9's direct non-health report-egress router |
...
| `app/` | Track F (Phase 0) | The canonical cross-platform client per 10 §10.1: pnpm workspace + its own cargo workspace (excluded from the root one). `src/features/{tx,analysis,handoff}` are three separate TypeScript projects — the 10 §10.2 firewall is a compilation boundary, not a lint. `packages/chain-client` is the sole home of `Finalized<T>`. `packages/{contexts,intents,receipts,llm-handoff}` + `skills/` are the D-21 serverless LLM handoff: files, clipboard and OS share only, **no MCP in any form** and no network primitive at all |
...
| `fuzz/` | S2 + N7 + N8 | `bleavit-fuzz` — five cargo-fuzz targets: `payload_scale_decode` (execution-guard `Payload` decode + guard invariants), `nested_wrapper_filter` (SafetyFilter I-10/I-11 differential vs an independent 06 §3.3 oracle), `lmsr_trade_paths` (market-core I-12 drain bound), `service_settlement_paths` (N7 two-terminal-state closure), and `xcm_client_ingress` (N8 positional matcher vs an independent spec-written oracle, including all nine inner-program carriers). A **separate nightly-pinned cargo workspace** (root `exclude = ["keeper", "fuzz"]`, own `rust-toolchain.toml`): libFuzzer + nightly must never perturb the runtime workspace's `=`-exact stable2606 pins — the `keeper/` precedent. Curated seed corpora under `fuzz/corpus/`; `tools/ci/fuzz-gates.sh` (CI job `fuzz`) runs fmt/clippy/oracle-tests + build + corpus-regression + a short random smoke. Long campaigns/distillation are B8 |

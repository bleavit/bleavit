...
| `pallets/` | Track A + B4 residual + N4 | Production FRAME shells over the frame-free cores; `pallets/client-registry` is N4's exact-`Location`, native-bonded external-client roster. `pallets/inflow-caps` remains the deliberate state-only exception |
| `runtime/` | B1–B12 + B15–B19 + N4 done/residual | `runtime/bleavit-runtime` is the real Cumulus parachain runtime (`construct_runtime!`, `impl_runtime_apis!`, `BaseCallFilter = SafetyFilter`). Frozen custom slots now include `Epoch` 61, `ExecutionGuard` 62, `InflowCaps` 63, `TrackOrigins` 64, and `ClientRegistry` 65. Existing wiring includes six-track governance, live Params consumers, native holds, treasury/XCM posture, generated weights and the 11-method `FutarchyApi` |

...
| `tools/release/` | B8 done | Release-artifact publication tooling (02 §11; 15 §5): reproducible-recipe runtime build, booted-node metadata extraction with `:code`↔wasm binding, the 02 critical-surface manifest + deterministic chainHead fixture recorder, content-addressed assembly with readiness report, the `bleavit.env-evidence.v1` contract B7 must produce, `environments.json` live-env inventory — see `tools/release/README.md`; a real tag release still fails closed on missing per-release B7 evidence, SQ-101, the B1b compliance gaps (PLAN SQ-140…SQ-150) and SQ-163 (the unwired oracle→treasury reserve-health seam) — the last two via the manifest's `release_blockers` rows; B2 closed the `FutarchyApi`/metadata-constant gaps |
...
| `runtime/` | B1a + B6 + B2 done; B1b review-open | `runtime/bleavit-runtime` is the real Cumulus parachain runtime (`construct_runtime!`, `impl_runtime_apis!`, `BaseCallFilter = SafetyFilter`, genesis presets with the 08 §2.1 VIT allocation/vesting). `ExecutionGuard` is live at index 62 with the full B6 upgrade path. B1b wires `Epoch` at frozen index 61, the real epoch clock/sovereign origins, market reads/opening, I-9 enqueue/callback flow and classifier/e2e coverage; canonical gates are green, but PLAN keeps B1b 🔨 for the compliance blockers and fail-closed backing gaps in SQ-140…SQ-150. B2 implemented all 11 `FutarchyApi` methods (`views.rs` + `impl_runtime_apis!`) against contract v4. |
